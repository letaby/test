// Decompiled with JetBrains decompiler
// Type: SapiLayer1.CommunicationsStateValueEventArgs
// Assembly: SAPI, Version=1.29.837.0, Culture=neutral, PublicKeyToken=2f6b11bb555a207b
// MVID: 18016536-4FB3-411C-A763-231E25A150EB
// Assembly location: C:\Users\petra\Downloads\Архив (2)\SAPI.dll

using System;

#nullable disable
namespace SapiLayer1;

public sealed class CommunicationsStateValueEventArgs : EventArgs
{
  private CommunicationsStateValue communicationsStateValue;

  internal CommunicationsStateValueEventArgs(CommunicationsStateValue cs)
  {
    this.communicationsStateValue = cs;
  }

  public CommunicationsStateValue CommunicationsStateValue => this.communicationsStateValue;
}
