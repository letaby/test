...
public class UserPanel : CustomPanel
{
	...
	private bool Busy => Online && cpc.CommunicationsState != CommunicationsState.Online;
	...
	{
		if (SetCPC(((CustomPanel)this).GetChannel("CPC302T", (ChannelLookupOptions)5)) | SetMCM(((CustomPanel)this).GetChannel("MCM21T")))
		{
	...
	private bool SetCPC(Channel cpc)
	{
		...
		if (this.cpc != cpc)
		{
			...
			if (this.cpc != null)
			{
				...
				this.cpc.Parameters.ParametersWriteCompleteEvent -= OnParametersWriteComplete;
				this.cpc.Parameters.ParametersReadCompleteEvent -= OnParametersReadComplete;
			...
			{
				this.cpc.CommunicationsStateUpdateEvent += OnChannelStateUpdate;
				cruisePowerParameter = this.cpc.Parameters["ptconf_p_Veh_FlcSelPar_u8"];
				cruisePowerParameter.ParameterUpdateEvent += OnCruisePowerParameterUpdate;
				this.cpc.Parameters.ParametersWriteCompleteEvent += OnParametersWriteComplete;
				this.cpc.Parameters.ParametersReadCompleteEvent += OnParametersReadComplete;
				if (CanRead)
	...
	private void UpdateUserInterface()
	{
		...
		bool flag3 = HaveReadCruisePower && cruisePowerParameter.Choices.Count > 3 && !Busy;
	...
	private void ReadRating()
	{
		...
		{
			ClearResults();
			ReportResult(Resources.Message_ReadingSettingFromCPC);
			cpc.Parameters.ReadGroup(cruisePowerParameter.GroupQualifier, fromCache: true, synchronous: false);
		}
	}
	...
	private void OnWriteRatingClick(object sender, EventArgs e)
	{
		if (CanWrite)
		{
			...
			if (Unlock(cpc))
			{
				ReportResult(Resources.Message_SendingSettingToCPC);
	...
	private void OnCruisePowerClick(object sender, EventArgs e)
	{
		if (cruisePowerParameter != null)
		{
			cruisePowerParameter.Value = cruisePowerParameter.Choices[3];
			UpdateUserInterface();
		}
	}
