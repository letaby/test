...
public class UserPanel : CustomPanel
{
	...
	private CalibrationStep[] steeringAngleCalibrationSteps = new CalibrationStep[15]
	{
		...
		new CalibrationStep(Resources.Message_EnsureVehicleIsInPtReadyState, Resources.Message_ClickNextToContinueToCalibration, CalibrationActions.EnsurePtReadyState, buttonEnabled: true),
	...
	public override void OnChannelsChanged()
	{
		...
		SetChannel(((CustomPanel)this).GetChannel(EcpcName), ref ecpcChannel);
		SetChannel(((CustomPanel)this).GetChannel(J19390Name), ref j19390Channel);
	...
	private void UpdateUI()
	{
		...
		if (currentStep != null)
		{
			...
			bool flag = currentStep.ButtonEnabled && !logFileIsOpen && !HasActiveFaults && InstrumentsAreInitialized && IsChannelConnected(channel) && IsChannelConnected(vrduChannel) && IsChannelConnected(absChannel) && IsChannelConnected(ecpcChannel);
			buttonNext.Enabled = ((currentStep.Action != CalibrationActions.EnsurePtReadyState) ? flag : (flag && PtReady));
		...
		{
			buttonNext.Enabled = !logFileIsOpen && PtReady && !HasActiveFaults;
		}
	...
	private string GetConnectEcusMessage()
	{
		...
		if (!IsChannelConnected(ecpcChannel))
		{
			arrayList.Add(EcpcName);
		}
		if (!IsChannelConnected(j19390Channel))
		{
			arrayList.Add(J19390Name);
		}
		...
		{
			text = string.Format(Resources.MessageFormat_Connect01234ToBeginTheCalibration, arrayList[0], arrayList[1], arrayList[2], arrayList[3], arrayList[4]);
		}
	...
	private void CheckIfPtReady()
	{
		...
		PtReady = (int)digitalReadoutInstrumentEngineState.RepresentedState == 1;
		if (PtReady)
		{
			PerformNextStep();
		}
	...
	{
		if (currentStep != null && currentStep.Action == CalibrationActions.EnsurePtReadyState)
		{
			CheckIfPtReady();
		}
	}
	...
	private void InitializeComponent()
	{
		...
		((AxisSingleInstrumentBase)dialInstrumentRightEndstop).PreferredAxisRange = new AxisRange(-1200.0, 1.0, "");
		...
		((AxisSingleInstrumentBase)dialInstrument2).PreferredAxisRange = new AxisRange(-1.0, 1200.0, "");
		...
		digitalReadoutInstrumentEngineState.Gradient.Initialize((ValueState)0, 4);
		digitalReadoutInstrumentEngineState.Gradient.Modify(1, 0.0, (ValueState)3);
		digitalReadoutInstrumentEngineState.Gradient.Modify(2, 1.0, (ValueState)1);
		...
		((SingleInstrumentBase)digitalReadoutInstrumentEngineState).Instrument = new Qualifier((QualifierTypes)1, "J1939-0", "DT_7315");
