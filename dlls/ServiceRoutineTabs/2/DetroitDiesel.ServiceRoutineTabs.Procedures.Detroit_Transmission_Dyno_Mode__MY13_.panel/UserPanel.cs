...
public class UserPanel : CustomPanel
{
	...
	private bool EngineIsRunning
	{
		get
		{
			...
			if (instrumentCurrentValue != null && int.TryParse(instrumentCurrentValue.ToString(), out var result))
			{
				return result > 0;
			}
			return false;
	...
	{
		SetCPC(((CustomPanel)this).GetChannel(CpcName));
	...
	private void SetCPC(Channel cpc)
	{
		...
		{
			this.cpc.CommunicationsStateUpdateEvent += OnChannelStateUpdate;
			engineSpeedInstrument = cpc.Instruments[EngineSpeedInstrumentQualifier];
			if (engineSpeedInstrument != null)
			{
				engineSpeedInstrument.InstrumentUpdateEvent += EngineSpeedUpdateEvent;
			}
	...
	{
		Service service = ((CustomPanel)this).GetService(CpcName, DynoModeTestStopServiceQualifier);
		if (service != null)
		{
	...
	private void ContinueTest()
	{
		...
		Service service = ((CustomPanel)this).GetService(CpcName, DynoModeTestStartServiceQualifier);
		if (service != null)
		{
			service.InputValues[0].Value = service.InputValues[0].Choices.GetItemFromRawValue(1);
			service.InputValues[1].Value = service.InputValues[1].Choices.GetItemFromRawValue(num);
	...
	private void InitializeComponent()
	{
		...
		((SingleInstrumentBase)barInstrument1).Instrument = new Qualifier((QualifierTypes)1, "virtual", "engineSpeed");
		...
		((TableLayoutPanel)(object)tableLayoutPanel3).Controls.Add((Control)(object)digitalReadoutInstrument4, 2, 0);
		...
		digitalReadoutInstrument4.FontGroup = "TDMReadouts";
		((SingleInstrumentBase)digitalReadoutInstrument4).FreezeValue = false;
		((SingleInstrumentBase)digitalReadoutInstrument4).Instrument = new Qualifier((QualifierTypes)1, "CPC04T", "DT_DSL_Accelerator_Pedal_Kick_Down_Status");
