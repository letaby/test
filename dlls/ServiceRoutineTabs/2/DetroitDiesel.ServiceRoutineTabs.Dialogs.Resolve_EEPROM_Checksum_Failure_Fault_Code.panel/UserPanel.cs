...
public class UserPanel : CustomPanel
{
	...
	private bool FaultIsPresent
	{
		get
		{
			if (Online && cpc2.FaultCodes.HaveBeenReadFromEcu)
			{
				FaultCode faultCode = cpc2.FaultCodes["740202"];
				if (faultCode != null)
				{
	...
	private bool EcuIsBusy => Online && cpc2.CommunicationsState != CommunicationsState.Online;
	...
	protected override void OnLoad(EventArgs e)
	{
		...
		SetCPC2(((CustomPanel)this).GetChannel("CPC02T"));
	}
	...
	private void OnParametersReadComplete(object sender, ResultEventArgs e)
	{
		...
		if (e.Succeeded)
		{
			...
			case Stage.ReadParameters:
				parameters.Clear();
				foreach (Parameter parameter in cpc2.Parameters)
				{
	...
	private void OnServiceComplete(object sender, ResultEventArgs e)
	{
		...
		cpc2.Services.ServiceCompleteEvent -= OnServiceComplete;
		if (e.Succeeded)
		{
	...
	private void UpdateUserInterface()
	{
		...
		else
		{
			...
			labelDTC.Text = Resources.Message_TheCPC2IsNotOnlineSoFaultCodesCouldNotBeRead;
		}
	}
	...
	private void SetCPC2(Channel channel)
	{
		if (cpc2 != channel)
		{
			if (cpc2 != null)
			{
				cpc2.CommunicationsStateUpdateEvent -= OnCommunicationsStateUpdate;
				cpc2.FaultCodes.FaultCodesUpdateEvent -= OnFaultCodesUpdate;
	...
	private void PerformCurrentStage()
	{
		if (cpc2 != null && cpc2.Online)
		{
			...
			case Stage.ReadParameters:
				...
				cpc2.Parameters.ParametersReadCompleteEvent += OnParametersReadComplete;
				cpc2.Parameters.ReadAll(synchronous: false);
				break;
			case Stage.Reset:
				...
				if (Unlock())
				{
					...
					ExecuteService("RT_Reload_Original_CPC_Factory_Settings_All_of_EEPROM_minus_SVDO_Block_Start_Routine_Status;FN_KeyOffOnReset");
				}
				break;
			case Stage.ReadResetParameters:
			...
			case Stage.WriteParameters:
			{
				...
				if (num > 0)
				{
					...
					cpc2.EcuInfos["CO_VIN"].Read(synchronous: true);
	...
	private int AssignParameters()
	{
		...
		{
			Parameter parameter = cpc2.Parameters[parameter2.Key];
	...
	{
		cpc2.Services.ServiceCompleteEvent += OnServiceComplete;
		if (cpc2.Services.Execute("RT_Reload_Original_CPC_Factory_Settings_All_of_EEPROM_minus_SVDO_Block_Start_Routine_Status;FN_KeyOffOnReset", synchronous: false) == 0)
		{
			cpc2.Services.ServiceCompleteEvent -= OnServiceComplete;
	...
	private bool Unlock()
	{
		try
		{
			cpc2.Extension.Invoke("Unlock", null);
			return true;
		}
	...
	private void StopWork(Reason reason)
	{
		...
		else
		{
			...
			case Reason.Disconnected:
				ReportResult(Resources.Message_TheCPC2WasDisconnected);
				break;
