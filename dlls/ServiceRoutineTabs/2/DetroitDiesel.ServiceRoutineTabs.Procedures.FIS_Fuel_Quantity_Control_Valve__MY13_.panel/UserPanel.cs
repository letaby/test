...
public class UserPanel : CustomPanel
{
	...
	private void FmuTestRequestResultsResultErrorBitServiceCompleteEvent(object sender, ResultEventArgs e)
	{
		...
		if (e.Succeeded)
		{
			...
			if (serviceOutputValue.Value == serviceOutputValue.Choices.GetItemFromRawValue(0))
			{
				...
				((Control)(object)scalingLabelFmuStickTestResult).Text = Resources.Message_FMUIsNOTSticking;
			...
			else if (serviceOutputValue.Value == serviceOutputValue.Choices.GetItemFromRawValue(1))
			{
				...
				((Control)(object)scalingLabelFmuStickTestResult).Text = Resources.Message_FMUISSticking;
			...
			else if (serviceOutputValue.Value == serviceOutputValue.Choices.GetItemFromRawValue(255))
			{
				...
				((Control)(object)scalingLabelFmuStickTestResult).Text = Resources.Message_FMUSignalIsNotAvailable;
				FmuStickTestDisplayMessage(Resources.Message_FMUSignalIsNotAvailable);
			}
			else
			{
				scalingLabelFmuStickTestResult.RepresentedState = (ValueState)2;
				((Control)(object)scalingLabelFmuStickTestResult).Text = Resources.Message_FMUValueUnknown;
	...
	private void FmuStickTestUpdateInstrumentsStatus()
	{
		...
		{
			flag2 = false;
			text = Resources.Message_Offline;
		}
	...
	private void FmuStickTestUpdateFaultConditions()
	{
		...
		{
			((Control)(object)labelFmuStickTestFaultCodesStatus).Text = Resources.Message_Offline;
	...
	private void InitializeComponent()
	{
		...
		((SingleInstrumentBase)digitalReadoutInstrumentFuel_Metering_Unit_FMU_desired_current).Instrument = new Qualifier((QualifierTypes)1, "MCM21T", "DT_AS121_Quantity_Control_Valve_Desired_Current");
