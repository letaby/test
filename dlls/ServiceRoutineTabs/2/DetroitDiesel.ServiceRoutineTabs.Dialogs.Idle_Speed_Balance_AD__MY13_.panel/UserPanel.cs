...
public class UserPanel : CustomPanel
{
	...
	private string ServiceNotSupportedMessage = Resources.MessageFormat_TheConnectedMCM21TDoesNotSupportTheServiceRoutine;
	...
	private string adrReturnText = Resources.Message_TestWasNotRan;
	...
	private void FinishTest()
	{
		...
		StopTM();
		DetermineResults();
		AppendDisplayMessage((!success) ? ErrorMessage : (adrReturnValue ? TestPassedMessage : TestFailedMessage));
		UpdateTestReadiness();
	}
	...
	private void DetermineResults()
	{
		...
		if (success)
		{
			adrReturnValue = tuple.Item1.Count == 0;
			if (tuple.Item1.Any())
			{
				text = string.Join(", <br />", tuple.Item1.Select((int i) => string.Format(CultureInfo.InvariantCulture, Resources.MessageFormat_Cylinder0MayBeInFaultRN, i)));
			}
			if (tuple.Item2.Any())
			{
				text += string.Join(", <br />", tuple.Item2.Select((Tuple<int, int> i) => string.Format(CultureInfo.InvariantCulture, Resources.MessageFormat_Cylinder0MightBeCompensatingForCylinder1RN, i.Item1, i.Item2)));
			}
	...
	private void InitializeComponent()
	{
		...
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add(labelUserNote, 1, 7);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)tableLayoutPanelTimer, 1, 5);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add(testStatusLabel, 0, 9);
		...
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add(buttonClose, 7, 9);
