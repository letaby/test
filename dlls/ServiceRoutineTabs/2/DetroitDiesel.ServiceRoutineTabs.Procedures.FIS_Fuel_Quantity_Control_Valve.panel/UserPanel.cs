...
public class UserPanel : CustomPanel
{
	...
	private void FmuStickTestPollStatusServiceCompleteEvent(object sender, ResultEventArgs e)
	{
		...
		if (e.Succeeded)
		{
			...
			{
				fmuTestRunning = true;
				FmuStickTestDisplayMessage(Resources.Message_TheFMUStickTestHasStarted0);
			}
	...
	private void FmuTestRequestResultsResultErrorBitServiceCompleteEvent(object sender, ResultEventArgs e)
	{
		...
		{
			scalingLabelFmuStickTestResult.RepresentedState = (ValueState)2;
			((Control)(object)scalingLabelFmuStickTestResult).Text = string.Format(Resources.MessageFormat_FMUStickTestError02, e.Exception.Message);
			FmuStickTestDisplayMessage(string.Format(Resources.MessageFormat_FMUStickTestError03, e.Exception.Message));
		}
	}
	...
	{
		SetChannel(((CustomPanel)this).GetChannel("MCM02T"));
	...
	private void SetChannel(Channel channel)
	{
		if (this.channel != channel)
		{
			...
			if (this.channel != null)
			{
				...
				instrumentEngineState = ConnectInstrument(this.channel.Instruments, "DT_ASL004_Engine_State", OnIgnitionUpdateEvent);
				instrumentEngineSpeed = ConnectInstrument(this.channel.Instruments, "DT_ASL002_Engine_Speed", OnIgnitionUpdateEvent);
				...
				instrumentCoolantTemperature = ConnectInstrument(this.channel.Instruments, "DT_ASL005_Coolant_Temperature", OnInstrumentUpdateEvent);
				instrumentFuelTemperature = ConnectInstrument(this.channel.Instruments, "DT_ASL005_Fuel_Temperature", OnInstrumentUpdateEvent);
				instrumentRailPressure = ConnectInstrument(this.channel.Instruments, "DT_ASL001_Rail_Pressure", OnInstrumentUpdateEvent);
	...
	private void InitializeComponent()
	{
		...
		((SingleInstrumentBase)digitalReadoutInstrumentQuantity_Control_Valve_Adaptation_Positive).Instrument = new Qualifier((QualifierTypes)4, "MCM02T", "Quantity_Control_Valve_Adaptation_Positive");
		...
		((SingleInstrumentBase)digitalReadoutInstrumentQuantity_Control_Valve_Adaptation_Negative).Instrument = new Qualifier((QualifierTypes)4, "MCM02T", "Quantity_Control_Valve_Adaptation_Negative");
		...
		((SingleInstrumentBase)digitalReadoutInstrumentRailPressure).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS043_Rail_Pressure");
		...
		((SingleInstrumentBase)digitalReadoutInstrumentFuelMass).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS087_Actual_Fuel_Mass");
		...
		((SingleInstrumentBase)digitalReadoutInstrumentFuel_Metering_Unit_FMU_desired_current).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS121_Quantity_Control_Valve_Desired_Current");
		...
		((SingleInstrumentBase)digitalReadoutInstrumentQuantity_Control_Valve_Current).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS100_Quantity_Control_Valve_Current");
