...
public class UserPanel : CustomPanel
{
	...
	private bool SetMCM(Channel mcm)
	{
		...
		if (this.mcm != mcm)
		{
			...
			{
				this.mcm.CommunicationsStateUpdateEvent += OnChannelStateUpdate;
				engineSpeedInstrument = ((CustomPanel)this).GetInstrument("MCM02T", "DT_AS010_Engine_Speed");
				if (engineSpeedInstrument != null)
				{
					engineSpeedInstrument.InstrumentUpdateEvent += OnEngineSpeedUpdate;
				}
	...
	private void CommandValue()
	{
		...
		{
			Service service = ((CustomPanel)this).GetService("MCM02T", "RT_SR068_Control_IAT_Start_status");
			if (service != null)
			{
	...
	private void StopManipulation()
	{
		if (CanStop)
		{
			Service service = ((CustomPanel)this).GetService("MCM02T", "RT_SR068_Control_IAT_Stop");
			if (service != null)
			{
				service.ServiceCompleteEvent += OnServiceCompleteEvent;
	...
	private void InitializeComponent()
	{
		...
		((SingleInstrumentBase)itvActualInstrument).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS034_Throttle_Valve_Actual_Position");
		...
		((SingleInstrumentBase)vehicleCheckInstrument).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS010_Engine_Speed");
