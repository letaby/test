...
public class UserPanel : CustomPanel
{
	...
	private static readonly ValidationInformation SerialNumberValidation = new ValidationInformation(new Regex("12\\d{12}", RegexOptions.Compiled), Resources.Message_SerialNumberShouldBe14DigitsInLengthWithTheFormatOf12Xxxxxxxxxxxx);
	...
	{
		if (SetACM(((CustomPanel)this).GetChannel("ACM02T")) && SetMCM2(((CustomPanel)this).GetChannel("MCM02T")))
		{
	...
	private static bool HasUnsentChanges(Channel channel)
	{
		...
		{
			if ((!acmScrAccumulatorQualifiers.Contains(parameter.Name) || channel.Ecu.Name != "ACM02T") && !object.Equals(parameter.Value, parameter.OriginalValue))
			{
				result = true;
				break;
			}
	...
	private void ReadAccumulators(bool synchronous)
	{
		...
		foreach (string acmScrAccumulatorQualifier in acmScrAccumulatorQualifiers)
		{
			DataItem obj = DataItem.Create(new Qualifier((QualifierTypes)4, "ACM02T", acmScrAccumulatorQualifier), (IEnumerable<Channel>)SapiManager.GlobalInstance.ActiveChannels);
	...
	private void InitializeComponent()
	{
		...
		((SingleInstrumentBase)digitalReadoutInstrument1).Instrument = new Qualifier((QualifierTypes)4, "ACM02T", "Time_Above_SCR_Inlet_Temp_1_Hour");
		...
		((SingleInstrumentBase)digitalReadoutInstrument2).Instrument = new Qualifier((QualifierTypes)4, "ACM02T", "Time_Above_SCR_Inlet_Temp_1_Min");
		...
		((SingleInstrumentBase)digitalReadoutInstrument3).Instrument = new Qualifier((QualifierTypes)4, "ACM02T", "Time_Above_SCR_Inlet_Temp_1_Sec");
		...
		((SingleInstrumentBase)digitalReadoutInstrument4).Instrument = new Qualifier((QualifierTypes)4, "ACM02T", "Time_Above_SCR_Inlet_Temp_2");
		...
		((SingleInstrumentBase)digitalReadoutInstrument5).Instrument = new Qualifier((QualifierTypes)4, "ACM02T", "Time_Above_SCR_Outlet_Temp");
