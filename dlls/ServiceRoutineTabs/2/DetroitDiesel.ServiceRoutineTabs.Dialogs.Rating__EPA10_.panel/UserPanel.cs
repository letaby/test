...
namespace DetroitDiesel.ServiceRoutineTabs.Dialogs.Rating__EPA10_.panel;
...
public class UserPanel : CustomPanel
{
	...
	private bool CanRead => Online && !Working && !Busy && !HaveReadCruisePower;

	private bool CanWrite => Online && !Working && !Busy && HaveReadCruisePower && !object.Equals(cruisePowerParameter.Value, cruisePowerParameter.OriginalValue);

	private bool CanClose => !Working;

	private bool Online => cpc2 != null && cpc2.Online;

	private bool HaveReadDescriptions => mcm != null && mcm.Online && mcm.CommunicationsState != CommunicationsState.ReadEcuInfo;
	...
	public UserPanel()
	{
		...
		buttonReadRating.Click += OnReadRatingClick;
	...
	{
		if (SetCPC2(((CustomPanel)this).GetChannel("CPC02T")) | SetMCM(((CustomPanel)this).GetChannel("MCM02T")))
		{
			UpdateWarningMessage();
		}
	}
	...
	private void UpdateUserInterface()
	{
		...
		if (HaveReadDescriptions)
		{
			...
			((Control)(object)labelLowPowerDescription).Text = string.Format(Resources.MessageFormat_Power01Torque231, text2, EcuInfoValue(mcm, "DT_STO_ID_Rated_engine_speed_for_rat_1_Rated_engine_speed_for_rat_1"), EcuInfoValue(mcm, "DT_STO_ID_Maximum_Engine_Torque_Maximum_Engine_Torque"), EcuInfoValue(mcm, "DT_STO_ID_Maximum_Torque_Speed_Maximum_Torque_Speed"));
		...
		buttonReadRating.Enabled = canRead;
	...
	private void OnParametersReadComplete(object sender, ResultEventArgs e)
	{
		...
		cpc2.Parameters.ParametersReadCompleteEvent -= OnParametersReadComplete;
		Working = false;
	}
	...
	private void OnParametersWriteComplete(object sender, ResultEventArgs e)
	{
		if (e.Succeeded)
		{
			ReportResult(Resources.Message_SettingSuccessfullySent);
		...
		cpc2.Parameters.ParametersWriteCompleteEvent -= OnParametersWriteComplete;
		Working = false;
	}
	...
	private void OnReadRatingClick(object sender, EventArgs e)
	{
		if (CanRead)
		{
			Working = true;
			ClearResults();
			ReportResult(Resources.Message_ReadingSettingFromCPC2);
			cpc2.Parameters.ParametersReadCompleteEvent += OnParametersReadComplete;
	...
	private void OnWriteRatingClick(object sender, EventArgs e)
	{
		if (CanWrite)
		{
			...
			if (Unlock(cpc2))
			{
				ReportResult(Resources.Message_SendingSettingToCPC2);
				cpc2.Parameters.ParametersWriteCompleteEvent += OnParametersWriteComplete;
	...
	private void InitializeComponent()
	{
		...
		((TableLayoutPanel)(object)tableLayoutPanel2).Controls.Add(buttonReadRating, 1, 8);
		...
		componentResourceManager.ApplyResources(buttonReadRating, "buttonReadRating");
		buttonReadRating.Name = "buttonReadRating";
		buttonReadRating.UseCompatibleTextRendering = true;
		buttonReadRating.UseVisualStyleBackColor = true;
