...
public class UserPanel : CustomPanel
{
	...
	private static readonly List<SetupInformation> Setups = new List<SetupInformation>(new SetupInformation[10]
	{
	...
	private TimerMode timerMode = TimerMode.FaultReset;
	...
	public UserPanel()
	{
		...
		InitializeComponent();
		warningManager = new WarningManager(string.Empty, Resources.Message_IdleSpeedBalanceTest, seekTimeListView.RequiredUserLabelPrefix);
	...
	private void SetMCM(Channel mcm)
	{
		...
		{
			this.mcm.CommunicationsStateUpdateEvent += OnChannelStateUpdate;
			engineState = this.mcm.Instruments["DT_AS023_Engine_State"];
			if (engineState != null)
			{
				engineState.InstrumentUpdateEvent += OnEngineStateUpdate;
				UpdateEngineState();
	...
	private void UpdateTestReadiness()
	{
		...
		((Control)(object)tableLayoutPanelTimer).Visible = timerControlTimeRemaining.IsTimerRunning;
	}
	...
	private void UpdateConnectedEcuType()
	{
		...
		EquipmentType equipmentType = SapiManager.GlobalInstance.ConnectedEquipment.FirstOrDefault(delegate(EquipmentType et)
		{
		...
		connectedEcuType = ((equipmentType != EquipmentType.Empty) ? Setups.FirstOrDefault((SetupInformation si) => si.Name == ((EquipmentType)(ref equipmentType)).Name) : null);
		if (connectedEcuType != null)
		{
			ScalingLabel obj = scalingLabel5;
			Label obj2 = cylinderLabel5;
			ScalingLabel obj3 = scalingLabel6;
			bool flag = (((Control)(object)cylinderLabel6).Visible = connectedEcuType.CylinderCount == 6);
			flag = (((Control)(object)obj3).Visible = flag);
			flag = (((Control)(object)obj2).Visible = flag);
	...
	private void StartTest()
	{
		...
		{
			if (ValidTestCondition && mcm != null)
			{
				testRunning = true;
				AppendDisplayMessage(Resources.Message_StartingTest);
				AppendDisplayMessage(string.Format(Resources.MessageFormat_ClearingFaults0Seconds, 15));
				mcm.FaultCodes.Reset(synchronous: false);
				timerMode = TimerMode.FaultReset;
				timerControlTimeRemaining.Duration = new TimeSpan(0, 0, 15);
				timerControlTimeRemaining.Start();
			}
			else
			{
				UpdateTestReadiness();
	...
	private void timerControlTimeRemaining_TimerCountdownCompleted(object sender, EventArgs e)
	{
		switch (timerMode)
		{
		case TimerMode.FaultReset:
			StartService();
			break;
		case TimerMode.ConductingTest:
			FinishTest();
			break;
		}
	}
	...
	private void StartService()
	{
		if (mcm != null)
		{
			if (mcm.FaultCodes.Current.Count() > 0)
			{
				AppendDisplayMessage(Resources.Message_WarningToEnsureProperResultsAllActiveFaultCodesShouldBeDiagnosedPriorRunningAnIdleSpeedBalanceTest);
			}
			AppendDisplayMessage(Resources.Message_ResettingCounters);
			Service service = mcm.Services["RT_SR0C5_Reset_ISC_Counter_in_ISC_Modul_Start"];
			if (service != null)
			{
				StartTM();
				service.ServiceCompleteEvent += OnResetIscCounterComplete;
				service.Execute(synchronous: false);
			}
			else
			{
				AppendDisplayMessage(string.Format(ServiceNotSupportedMessage, "RT_SR0C5_Reset_ISC_Counter_in_ISC_Modul_Start"));
				testRunning = false;
				UpdateTestReadiness();
			}
		}
	...
	{
		if (mcm != null && connectedEcuType != null && !string.IsNullOrEmpty(connectedEcuType.ThermalManagementModeStartServiceName))
		{
			Service service = mcm.Services[connectedEcuType.ThermalManagementModeStartServiceName];
			if (service != null)
			{
				service.InputValues[0].Value = service.InputValues[0].Choices.GetItemFromRawValue(5);
				service.ServiceCompleteEvent += OnStartTmComplete;
				service.Execute(synchronous: false);
				return true;
			}
		}
	...
	private void OnStartTmComplete(object sender, ResultEventArgs e)
	{
		...
		{
			success = false;
			AppendDisplayMessage(Resources.Message_CannotSetThermalManagementMode);
			if (e.Exception != null && e.Exception.Message != null)
			{
				AppendDisplayMessage(e.Exception.Message);
			}
			testRunning = false;
		}
		else
		{
			AppendDisplayMessage(Resources.Message_ThermalManagementModeSet);
		}
	}

	private void StopTM()
	{
		if (mcm != null && connectedEcuType != null && !string.IsNullOrEmpty(connectedEcuType.ThermalManagementModeStopServiceName))
		{
			Service service = mcm.Services[connectedEcuType.ThermalManagementModeStopServiceName];
			if (service != null)
			{
				service.ServiceCompleteEvent += OnStopTmComplete;
				service.Execute(synchronous: false);
			}
		}
	}
	...
	private void OnStopTmComplete(object sender, ResultEventArgs e)
	{
		...
		if (!e.Succeeded)
		{
			AppendDisplayMessage(Resources.Message_CannotStopThermalManagementMode);
			if (e.Exception != null && e.Exception.Message != null)
			{
				AppendDisplayMessage(e.Exception.Message);
			}
		...
		{
			AppendDisplayMessage(Resources.Message_ThermalManagementModeStopped);
		}
	}
	...
	private void OnResetIscCounterComplete(object sender, ResultEventArgs e)
	{
		...
		{
			success = false;
			AppendDisplayMessage(Resources.Message_ResettingTheISCCounterServiceRoutineFailedToRun);
			if (e.Exception != null && e.Exception.Message != null)
			{
				AppendDisplayMessage(e.Exception.Message);
			}
		...
		{
			AppendDisplayMessage(Resources.Message_TheISCCounterHasBeenResetTheTestContinues);
			success = true;
			cylinder = 0;
			Advance();
		}
	...
	private void Advance()
	{
		...
		if (service != null)
		{
			if (++cylinder <= connectedEcuType.CylinderCount && testRunning)
			{
			...
			{
				timerMode = TimerMode.ConductingTest;
				timerControlTimeRemaining.Duration = new TimeSpan(0, 0, 90);
				timerControlTimeRemaining.Start();
				AppendDisplayMessage(string.Format(Resources.MessageFormat_RunningTestFor0Seconds, 90));
			}
		}
		else
		{
	...
	private void FinishTest()
	{
		testRunning = false;
		StopTM();
		Tuple<List<int>, List<Tuple<int, int>>> tuple = HighlightValues();
		bool flag = tuple.Item1.Count == 0;
		AppendDisplayMessage((!success) ? ErrorMessage : (flag ? TestPassedMessage : TestFailedMessage));
		UpdateTestReadiness();
	}
	...
	private Tuple<List<int>, List<Tuple<int, int>>> HighlightValues()
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			if (num > 69.5 || num < -69.5)
			{
				int num2 = connectedEcuType.GetPreviousCylinder(i + 1) - 1;
				double num3 = list[num2].Key.ValueAsDouble(list[num2].Key.Value);
				if (num3 > 69.5 || num3 < -69.5)
				{
		...
		return new Tuple<List<int>, List<Tuple<int, int>>>(list2, list3);
	}
	...
	private void InitializeComponent()
	{
		...
		tableLayoutPanelTimer = new TableLayoutPanel();
		labelTimeRemaining = new Label();
		timerControlTimeRemaining = new TimerControl();
		labelUserNote = new System.Windows.Forms.Label();
		((Control)(object)tableLayoutPanel1).SuspendLayout();
		((Control)(object)tableLayoutPanel2).SuspendLayout();
		((Control)(object)tableLayoutPanel3).SuspendLayout();
		((Control)(object)tableLayoutPanelTimer).SuspendLayout();
		((Control)this).SuspendLayout();
		componentResourceManager.ApplyResources(tableLayoutPanel1, "tableLayoutPanel1");
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)connectionCheck, 0, 1);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)vehicleCheckInstrument, 3, 1);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)engineIdlingStatus, 1, 2);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)testReadyCheck, 0, 5);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)mcmConnectionStatus, 1, 1);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)title, 0, 0);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)tableLayoutPanel2, 0, 9);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)temperatureCheck, 0, 3);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)temperatureStatus, 1, 3);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)tableLayoutPanel3, 4, 1);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)vehicleStatusCheck, 0, 4);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)vehicleStatus, 1, 4);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add(buttonExecute, 1, 5);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)idlingCheck, 0, 2);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)seekTimeListView, 2, 5);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add((Control)(object)tableLayoutPanelTimer, 1, 6);
		((TableLayoutPanel)(object)tableLayoutPanel1).Controls.Add(labelUserNote, 1, 8);
		...
		((SingleInstrumentBase)vehicleCheckInstrument).Instrument = new Qualifier((QualifierTypes)1, "MCM21T", "DT_DS019_Vehicle_Check_Status");
		...
		((TableLayoutPanel)(object)tableLayoutPanel1).SetColumnSpan((Control)(object)title, 7);
		componentResourceManager.ApplyResources(title, "title");
		title.FontGroup = null;
		title.LineAlignment = StringAlignment.Center;
		((Control)(object)title).Name = "title";
		...
		componentResourceManager.ApplyResources(tableLayoutPanelTimer, "tableLayoutPanelTimer");
		((TableLayoutPanel)(object)tableLayoutPanelTimer).Controls.Add((Control)(object)labelTimeRemaining, 0, 0);
		((TableLayoutPanel)(object)tableLayoutPanelTimer).Controls.Add((Control)(object)timerControlTimeRemaining, 0, 1);
		((Control)(object)tableLayoutPanelTimer).Name = "tableLayoutPanelTimer";
		labelTimeRemaining.Alignment = StringAlignment.Near;
		componentResourceManager.ApplyResources(labelTimeRemaining, "labelTimeRemaining");
		((Control)(object)labelTimeRemaining).Name = "labelTimeRemaining";
		labelTimeRemaining.Orientation = (TextOrientation)1;
		componentResourceManager.ApplyResources(timerControlTimeRemaining, "timerControlTimeRemaining");
		timerControlTimeRemaining.Duration = TimeSpan.Parse("00:00:15");
		timerControlTimeRemaining.FontGroup = null;
		((Control)(object)timerControlTimeRemaining).Name = "timerControlTimeRemaining";
		((Control)(object)timerControlTimeRemaining).TabStop = false;
		timerControlTimeRemaining.TimerCountdownCompletedDisplayMessage = null;
		timerControlTimeRemaining.TimerCountdownCompleted += timerControlTimeRemaining_TimerCountdownCompleted;
		componentResourceManager.ApplyResources(labelUserNote, "labelUserNote");
		((TableLayoutPanel)(object)tableLayoutPanel1).SetColumnSpan((Control)labelUserNote, 4);
		labelUserNote.ForeColor = Color.Red;
		labelUserNote.Name = "labelUserNote";
		labelUserNote.UseCompatibleTextRendering = true;
		...
		((Control)(object)tableLayoutPanelTimer).ResumeLayout(performLayout: false);
		((Control)(object)tableLayoutPanelTimer).PerformLayout();
