...
public class UserPanel : CustomPanel
{
	...
	private void UpdateUserInterface()
	{
		...
		string text = Resources.Message_TestCanStart0;
		if (!buttonStart.Enabled)
		{
			if (TestRunning)
			{
				...
				case WaitState.WaitingToInitiateRunOffRampUp:
					...
					break;
				case WaitState.WaitingForRampUp:
					text = string.Format(CultureInfo.CurrentCulture, Resources.MessageFormat_WaitingForEngineSpeedToReach0Rpm1, 1200);
					break;
				case WaitState.WaitingForRunOffRampUp:
					text = string.Format(CultureInfo.CurrentCulture, Resources.MessageFormat_WaitingForEngineSpeedToReach0Rpm, 1800);
					break;
				case WaitState.WaitingForThermalCondition:
				{
	...
	private void InitializeComponent()
	{
		...
		((SingleInstrumentBase)barInstrumentCoolantTemp).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS013_Coolant_Temperature");
		...
		((SingleInstrumentBase)barInstrumentEGRDeltaPressure).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS025_EGR_Delta_Pressure");
		...
		((SingleInstrumentBase)barInstrumentEGRCommandedValue).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS031_EGR_Commanded_Governor_Value");
		...
		((SingleInstrumentBase)barInstrumentChargeAirCoolerOutTemp).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS060_Charge_Air_Cooler_Outlet_Temperature");
		...
		((SingleInstrumentBase)digitalReadoutFault).Instrument = new Qualifier((QualifierTypes)32, "MCM02T", "630A12");
		...
		((SingleInstrumentBase)digitalReadoutVehicleCheckStatus).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_DS019_Vehicle_Check_Status");
		...
		((SingleInstrumentBase)digitalReadoutEngineSpeed).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS010_Engine_Speed");
