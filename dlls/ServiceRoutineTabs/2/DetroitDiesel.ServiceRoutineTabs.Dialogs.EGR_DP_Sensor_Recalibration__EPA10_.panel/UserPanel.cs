...
public class UserPanel : CustomPanel
{
	...
	{
		bool flag = false;
		string text = Resources.Message_MCM2IsNotConnected;
		if (mcm != null)
		{
			if (mcm.CommunicationsState == CommunicationsState.Online)
			{
				text = Resources.Message_MCM2IsConnected;
				flag = true;
			}
			else
			{
				text = Resources.Message_MCM2IsBusy;
			}
		}
	...
	private void OnServiceComplete(object sender, ResultEventArgs e)
	{
		...
		if (e.Succeeded)
		{
			...
			ReportResult(string.Format(Resources.MessageFormat_RecalibrationExecuted, serviceOutputValue.Value.ToString()));
		}
		else
		{
