...
public class UserPanel : CustomPanel
{
	...
	{
		bool flag = false;
		string text = Resources.Message_MCM21TIsNotConnected;
		if (mcm != null)
		{
			if (mcm.CommunicationsState == CommunicationsState.Online)
			{
				text = Resources.Message_MCM21TIsConnected;
				flag = true;
			}
			else
			{
				text = Resources.Message_MCM21TIsBusy;
			}
		}
	...
	private void OnServiceComplete(object sender, ResultEventArgs e)
	{
		...
		{
			ReportResult(string.Format(Resources.MessageFormat_AnErrorOccurredDuringRecalibration, e.Exception.Message));
		}
