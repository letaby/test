...
public class UserPanel : CustomPanel
{
	...
	private bool Online => cpc2 != null && cpc2.CommunicationsState == CommunicationsState.Online;
	...
	private void SetCPC2(Channel cpc2)
	{
		...
		{
			this.cpc2.CommunicationsStateUpdateEvent += OnChannelStateUpdate;
			ecuInfo = this.cpc2.EcuInfos["CO_RealTimeClock"];
			if (ecuInfo != null)
			{
				ecuInfo.EcuInfoUpdateEvent += OnEcuTimeUpdate;
			}
	...
	private void SetRealTimeClock(DateTime dateTime)
	{
		...
		{
			ClearResults();
			service = ((CustomPanel)this).GetService("CPC02T", "DL_ID_Real_Time_Clock");
			if (service != null)
			{
				UpdateUserInterface();
				...
				ReportResult(string.Format(Resources.MessageFormat_SettingRealTimeClockTo01, dateTime.ToString("yyyy/MM/dd HH:mm:ss"), empty, dateTime2.ToString("yyyy/MM/dd HH:mm:ss")));
	...
	private void OnServiceComplete(object sender, ResultEventArgs e)
	{
		...
		{
			ReportResult(string.Format(Resources.MessageFormat_AnErrorOccurredWhileSettingTheRealTimeClock0, e.Exception.Message));
		}
		service = null;
