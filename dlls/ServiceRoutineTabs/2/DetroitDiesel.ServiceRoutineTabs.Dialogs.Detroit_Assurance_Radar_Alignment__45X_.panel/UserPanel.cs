...
public class UserPanel : CustomPanel
{
	...
	private Channel Rdf03tChannel
	{
		get
		{
			return rdf03tChannel;
		}
		set
		{
			if (rdf03tChannel != value)
			{
				warningManager.Reset();
				if (rdf03tChannel != null)
				{
					rdf03tChannel.Parameters.ParametersReadCompleteEvent -= Rdf03t_ParametersReadCompleteEvent;
				}
				rdf03tChannel = value;
				if (rdf03tChannel != null)
				{
	...
	public override void OnChannelsChanged()
	{
		if (!closing)
		{
			Rdf03tChannel = ((CustomPanel)this).GetChannel("RDF03T");
		}
		((CustomPanel)this).OnChannelsChanged();
	}
	...
	{
		if (Rdf03tChannel != null && Rdf03tChannel.CommunicationsState == CommunicationsState.Online && Rdf03tChannel.Parameters["VertPos"] != null && !Rdf03tChannel.Parameters["VertPos"].HasBeenReadFromEcu)
		{
			Rdf03tChannel.Parameters.ReadGroup(Rdf03tChannel.Parameters["VertPos"].GroupQualifier, fromCache: true, synchronous: false);
		}
	}
	...
	private void InitializeComponent()
	{
		...
		((AxisSingleInstrumentBase)dialInstrumentVehicleSpeed).Gradient.Modify(1, 0.0, (ValueState)1);
		...
		((SingleInstrumentBase)barInstrumentProcedureProgress).Instrument = new Qualifier((QualifierTypes)64, "RDF03T", "RT_Service_Alignment_Request_Results_Progress");
		...
		seekTimeListView1.RequiredUserLabelPrefix = "Detroit Assurance Driving Radar Alignment 45X";
		...
		((SingleInstrumentBase)digitalReadoutInstrument1).Instrument = new Qualifier((QualifierTypes)1, "J1939-0", "DT_70");
		...
		((SingleInstrumentBase)digitalReadoutInstrumentVertPos).Instrument = new Qualifier((QualifierTypes)4, "RDF03T", "VertPos");
		...
		sharedProcedureSelection1.SharedProcedureQualifiers = new SubjectCollection((IEnumerable<string>)new string[1] { "SP_DrivingRadarAlignment_45X" });
