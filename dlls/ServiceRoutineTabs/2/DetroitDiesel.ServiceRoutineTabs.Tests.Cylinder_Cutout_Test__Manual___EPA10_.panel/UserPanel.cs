...
public class UserPanel : CustomPanel
{
	...
	private bool CanDecrement
	{
		...
		{
			int idleSpeed = IdleSpeed;
			return Online && (idleSpeed > 600 || idleSpeed == -1);
		}
	}
	...
	public UserPanel()
	{
		...
		buttonIdleSpeedApply.Click += OnApplyClick;
	...
	private void UpdateIdleSpeedUI()
	{
		...
		buttonIdleSpeedApply.Enabled = CanApply;
	...
	private void OnCylinderOnClick(object sender, EventArgs e)
	{
		...
		SwitchCylinder(Convert.ToInt32(button.Tag), on: true);
	}

	private void OnCylinderOffClick(object sender, EventArgs e)
	{
		Button button = sender as Button;
		SwitchCylinder(Convert.ToInt32(button.Tag), on: false);
	}
	...
	private void SwitchCylinder(int number, bool on)
	{
		...
		if (service != null)
		{
			...
			if (on)
			{
				ReportResult(string.Format(Resources.MessageFormat_EnablingCylinder0, number), withNewLine: false);
			}
			else
			{
				ReportResult(string.Format(Resources.MessageFormat_CuttingCylinder0, number), withNewLine: false);
			}
	...
	private void SetIdleSpeed()
	{
		...
		{
			Service service = mcm.Services["RT_SR015_Idle_Speed_Modification_Start"];
			if (service != null)
			{
				int idleSpeed = IdleSpeed;
				service.ServiceCompleteEvent += OnServiceCompleteEvent;
	...
	private void ResetIdleSpeed()
	{
		...
		{
			Service service = mcm.Services["RT_SR015_Idle_Speed_Modification_Stop"];
			if (service != null)
			{
				service.ServiceCompleteEvent += OnServiceCompleteEvent;
	...
	private void InitializeComponent()
	{
		...
		buttonIdleSpeedApply = new Button();
		...
		((SingleInstrumentBase)DigitalReadoutInstrument4).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_DS019_Vehicle_Check_Status");
		...
		((SingleInstrumentBase)DigitalReadoutInstrument1).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS013_Coolant_Temperature");
		...
		((SingleInstrumentBase)DigitalReadoutInstrument2).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS010_Engine_Speed");
		...
		((SingleInstrumentBase)DigitalReadoutInstrument3).Instrument = new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS003_Actual_Torque");
		...
		((Collection<Qualifier>)(object)chartInstrument.Instruments).Add(new Qualifier((QualifierTypes)1, "MCM02T", "DT_AS003_Actual_Torque"));
		...
		((TableLayoutPanel)(object)tableLayoutPanel2).Controls.Add(buttonIdleSpeedApply, 6, 1);
		((TableLayoutPanel)(object)tableLayoutPanel2).Controls.Add(buttonIdleSpeedReset, 7, 1);
		...
		componentResourceManager.ApplyResources(buttonIdleSpeedApply, "buttonIdleSpeedApply");
		buttonIdleSpeedApply.Name = "buttonIdleSpeedApply";
		buttonIdleSpeedApply.UseCompatibleTextRendering = true;
		buttonIdleSpeedApply.UseVisualStyleBackColor = true;
