...
public class UserPanel : CustomPanel
{
	...
	private class DisplayCounter
	{
		...
		internal bool Enabled
		{
			...
			set
			{
				...
				((Control)(object)outputText).Enabled = value;
			}
		}
	...
	private bool TestIsResetting
	{
		get
		{
			...
			case TestSteps.resetFuelCutoffValve:
			case TestSteps.resetHcDoserValve:
			case TestSteps.resetAllCyliners:
			case TestSteps.resetDisableHCDoserStart:
	...
	private void UpdateUserInterface()
	{
		...
		else
		{
			...
			case 0:
				...
				break;
			case 1:
				testCanStart = testStep == TestSteps.notRunning;
				stringBuilder.Append(TestIsRunning ? TestIsRunningString : TestIsReadyString);
				break;
			case 2:
				if (TestIsRunning)
				{
	...
	private string AutomaticFsicTestInstrumentsReadyToStartTest(ref bool testCanStart)
	{
		...
		else if ((int)digitalReadoutInstrumentCoolantTemperature.RepresentedState == 3)
		{
			...
			stringBuilder.AppendFormat(CultureInfo.CurrentCulture, Resources.MessageFormat_CannotStartBecauseTheCoolantTemperatureIs0, string.IsNullOrEmpty(minimumOKValueString) ? Resources.Message_TooLow : (Resources.Message_LessThan + minimumOKValueString));
		}
		else if ((int)digitalReadoutInstrumentFuelTemperature.RepresentedState == 3)
		{
			...
			stringBuilder.AppendFormat(CultureInfo.CurrentCulture, Resources.MessageFormat_CannotStartBecauseTheFuelTemperatureIs0, string.IsNullOrEmpty(minimumOKValueString) ? Resources.Message_TooLow1 : (Resources.Message_LessThan1 + minimumOKValueString));
		}
		else
		{
			testCanStart = true;
	...
	private void RunFuelSystemCheck()
	{
		...
		case TestSteps.testStarted:
			...
			RunFuelSystemCheck();
			break;
		case TestSteps.disableHCDoserStart:
			AdvanceTestStep();
			DisableHCDoserStart();
			break;
		case TestSteps.setHcDoserValve:
			AdvanceTestStep();
			CloseHCDoserValveStart();
			break;
		case TestSteps.setFuelCutoffValve:
			AdvanceTestStep();
			OpenFuelCutOffValveStart();
			break;
		...
		case TestSteps.cutAllCylinders:
		{
			...
			{
				flag = false;
				ReportResult(string.Format(CultureInfo.CurrentCulture, Resources.MessageFormat_Error01, ex.Message));
			}
			catch (CaesarException ex2)
			{
				flag = false;
				ReportResult(string.Format(CultureInfo.CurrentCulture, Resources.MessageFormat_Error0, ex2.Message));
			}
			if (!flag)
			{
		...
		case TestSteps.resetFuelCutoffValve:
			AdvanceTestStep();
			OpenFuelCutOffValveStop();
			break;
		case TestSteps.resetHcDoserValve:
			AdvanceTestStep();
			CloseHCDoserValveStop();
			break;
		case TestSteps.resetAllCyliners:
			AdvanceTestStep();
		...
		case TestSteps.manualTestStart:
			...
			RunFuelSystemCheck();
			break;
		case TestSteps.manualTestDisableHCDoserStart:
			AdvanceTestStep();
			DisableHCDoserStart();
			break;
		case TestSteps.manualTestSetHcDoserValve:
			AdvanceTestStep();
			CloseHCDoserValveStart();
			break;
		case TestSteps.manualTestSetFuelCutoffValve:
			AdvanceTestStep();
			OpenFuelCutOffValveStart();
			break;
	...
	private void WaitForEngineToStart()
	{
		...
		case EngineState.Unknown:
			...
			break;
		default:
			throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Resources.MessageFormat_EngineStateNotPossible0, EngineStatus));
		}
	}
	...
	private void CloseHCDoserValveStart()
	{
		...
		if (service != null)
		{
			ReportResult(Resources.Message_ClosingTheHCDoserValve);
			service.InputValues[0].Value = service.InputValues[0].Choices.GetItemFromRawValue(HCDoserPwmIndex);
			service.InputValues[1].Value = 0.0;
			service.InputValues[2].Value = (float)TestDuration.TotalMilliseconds;
			service.ServiceCompleteEvent += OnServiceCompleteEvent;
			service.Execute(synchronous: false);
		}
		else
		{
			ServiceNotFoundErrorMethod("RT_SR003_PWM_Routine_for_Production_Start_PWM_Value");
		}
	}
	...
	private void CloseHCDoserValveStop()
	{
		...
		if (service != null)
		{
			ReportResult(Resources.Message_ResettingHCDoserValve);
			service.InputValues[0].Value = service.InputValues[0].Choices.GetItemFromRawValue(HCDoserPwmIndex);
		...
		{
			ServiceNotFoundErrorMethod("RT_SR003_PWM_Routine_for_Production_Stop");
		}
	}
	...
	private void OpenFuelCutOffValveStart()
	{
		...
		if (service != null)
		{
			ReportResult(Resources.Message_OpeningFuelCutoffValve);
			service.InputValues[0].Value = service.InputValues[0].Choices.GetItemFromRawValue(FuelCutoffValveSwIndex);
		...
		{
			ServiceNotFoundErrorMethod("RT_SR005_SW_Routine_for_Production_Start_SW_Operation");
		}
	}
	...
	private void OnRepeatSwRoutineTimerTick(object sender, EventArgs args)
	{
		...
		else if (TestIsRunning)
		{
			...
			if (!TestIsResetting)
			{
				ReportResult(Resources.Message_CancellingTheTestUnableToKeepTheFuelCutoffValveOpen);
				CancelTest();
			}
		}
	}
	...
	private void OpenFuelCutOffValveStop()
	{
		...
		if (service != null)
		{
			ReportResult(Resources.Message_ResettingFuelCutoffValve);
			service.InputValues[0].Value = service.InputValues[0].Choices.GetItemFromRawValue(FuelCutoffValveSwIndex);
			service.ServiceCompleteEvent += OnServiceCompleteEventNoFail;
			service.Execute(synchronous: false);
		}
		...
		{
			ServiceNotFoundErrorMethod("RT_SR005_SW_Routine_for_Production_Stop");
		}
	}

	private void StopRepeatSwRoutineTimer()
	{
		if (repeatSwRoutineTimer != null)
		{
			repeatSwRoutineTimer.Stop();
			repeatSwRoutineTimer.Tick -= OnRepeatSwRoutineTimerTick;
		}
		if (repeatSwRoutineStopwatch != null)
		{
	...
	{
		Service service = ((CustomPanel)this).GetService("MCM", "RT_SR018_Disable_HC_Doser_Start");
		if (service != null)
		{
			service.ServiceCompleteEvent += OnServiceCompleteEvent;
			service.Execute(synchronous: false);
		...
		{
			ServiceNotFoundErrorMethod("RT_SR018_Disable_HC_Doser_Start");
		}
	}

	private void DisableHCDoserStop()
	{
		Service service = ((CustomPanel)this).GetService("MCM", "RT_SR018_Disable_HC_Doser_Stop");
		if (service != null)
		{
			service.ServiceCompleteEvent += OnServiceCompleteEventNoFail;
			service.Execute(synchronous: false);
		...
		{
			ServiceNotFoundErrorMethod("RT_SR018_Disable_HC_Doser_Stop");
		}
	}
	...
	private void StartManualFsicTest()
	{
		if (testStep == TestSteps.notRunning)
		{
			ReportResult(string.Format(CultureInfo.CurrentCulture, Resources.MessageFormat_StartingTheManualFSICTestTheHCDoserWillRemainClosedAndTheFuelCutoffValveOpenFor0 + "\r\n" + Resources.PressTheStopAllButtonToResetTheValves, DisplayTime(TestDuration)));
			testStep = TestSteps.manualTestStart;
			RunFuelSystemCheck();
		}
		UpdateUserInterface();
	}
	...
	private static string DisplayTime(TimeSpan time)
	{
		...
		if (time.Minutes == 1)
		{
			text2 = Resources.Message_1Minute;
		}
	...
	private void InitializeComponent()
	{
		...
		((SingleInstrumentBase)digitalReadoutInstrumentFuelCompensationPressure).Instrument = new Qualifier((QualifierTypes)1, "MCM", "DT_AS024_Fuel_Compensation_Pressure");
