...
public class UserPanel : CustomPanel
{
	...
	private static readonly List<SetupInformation> Setups = new List<SetupInformation>(new SetupInformation[6]
	{
		new SetupInformation("MBE 900 series", "MBE900", useFuelTemperature: true),
		new SetupInformation("MBE 4000 series", "MBE4000", useFuelTemperature: true),
		new SetupInformation("DD15", "DD15", useFuelTemperature: true),
		new SetupInformation("DD15 EURO4", "DD15EURO4", useFuelTemperature: true),
		new SetupInformation("DD13", "DD13", useFuelTemperature: true),
		new SetupInformation("Series 60", "S60", useFuelTemperature: false)
	});
	...
	private string ServiceNotSupportedMessage = Resources.MessageFormat_TheConnectedMCMDoesNotSupportTheServiceRoutine0;

	private string EcuNotConnectedMessage = Resources.Message_MCMIsNotConnected;

	private string EcuNotMbeMessage = Resources.Message_MCMIsConnectedButEngineTypeIsNotSupported;

	private string EcuReadyMessage = Resources.Message_MCMIsConnectedAndEngineTypeIsSupported;

	private string EcuBusyMessage = Resources.Message_MCMIsConnectedButIsBusy;
	...
	private string VehicleStatusCheckNotOkMessage = Resources.Message_TheTransmissionMustBeInNeutralAndTheParkingBrakeON0;

	private readonly Qualifier qualifier1 = new Qualifier((QualifierTypes)1, "MCM", "DT_Idle_Speed_Balance_Values_Cylinder_1");

	private readonly Qualifier qualifier2 = new Qualifier((QualifierTypes)1, "MCM", "DT_Idle_Speed_Balance_Values_Cylinder_2");

	private readonly Qualifier qualifier3 = new Qualifier((QualifierTypes)1, "MCM", "DT_Idle_Speed_Balance_Values_Cylinder_3");

	private readonly Qualifier qualifier4 = new Qualifier((QualifierTypes)1, "MCM", "DT_Idle_Speed_Balance_Values_Cylinder_4");

	private readonly Qualifier qualifier5 = new Qualifier((QualifierTypes)1, "MCM", "DT_Idle_Speed_Balance_Values_Cylinder_5");

	private readonly Qualifier qualifier6 = new Qualifier((QualifierTypes)1, "MCM", "DT_Idle_Speed_Balance_Values_Cylinder_6");
	...
	private bool TemperaturesAreInRange => FuelTemperatureIsInRange && CoolantTemperatureIsInRange;

	private bool FuelTemperatureIsInRange => connectedEcuType == null || !connectedEcuType.UseFuelTemperature || (int)fuelTemperatureInstrument.RepresentedState == 1;

	private bool CoolantTemperatureIsInRange => (int)coolantTemperatureInstrument.RepresentedState == 1;
	...
	private void UpdateTemperatureCheck()
	{
		...
		{
			temperatureCheck.CheckState = CheckState.Checked;
			text = Resources.Message_FuelAndCoolantTemperaturesAreInRange;
		}
		else
		{
			temperatureCheck.CheckState = CheckState.Unchecked;
			if (!FuelTemperatureIsInRange)
			{
			...
			if (!CoolantTemperatureIsInRange)
			{
	...
	private void HighlightValues()
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			{
				int index = firingOrder[i];
				double num2 = list[index].Key.ValueAsDouble(list[index].Key.Value);
				representedState = ((!(num2 > 99.5) && !(num2 < -99.5)) ? ((ValueState)3) : ((ValueState)2));
			}
	...
	private void InitializeComponent()
	{
		...
		coolantTemperatureInstrument.FontGroup = "idleSpeedBalancePreconditionsEPA07";
		...
		((SingleInstrumentBase)coolantTemperatureInstrument).Instrument = new Qualifier((QualifierTypes)1, "MCM", "DT_AS013_Coolant_Temperature");
		...
		fuelTemperatureInstrument.FontGroup = "idleSpeedBalancePreconditionsEPA07";
		...
		((SingleInstrumentBase)fuelTemperatureInstrument).Instrument = new Qualifier((QualifierTypes)1, "MCM", "DT_AS014_Fuel_Temperature");
		...
		vehicleCheckInstrument.FontGroup = "idleSpeedBalancePreconditionsEPA07";
		...
		((SingleInstrumentBase)vehicleCheckInstrument).Instrument = new Qualifier((QualifierTypes)1, "MCM", "DT_DS019_Vehicle_Check_Status");
		...
		((Control)(object)vehicleStatusCheck).BackColor = Color.Transparent;
		...
		((Control)(object)temperatureCheck).BackColor = Color.Transparent;
		...
		((TableLayoutPanel)(object)tableLayoutPanel1).SetColumnSpan((Control)(object)title, 5);
