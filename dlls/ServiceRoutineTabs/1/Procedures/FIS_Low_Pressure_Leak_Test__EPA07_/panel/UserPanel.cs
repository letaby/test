...
// Type: DetroitDiesel.ServiceRoutineTabs.Procedures.FIS_Low_Pressure_Leak_Test__EPA07_.panel.UserPanel
...
public class UserPanel : CustomPanel
{
  ...
  {
    this.SetSwRoutineStart((int) UserPanel.testDuration.TotalMilliseconds);
  }

  private void OpenHCDoserValve()
  {
    this.SetPwmRoutineStart(25, (int) UserPanel.OpenHCDoserControlTime.TotalMilliseconds);
  }

  private void CloseHCDoserValve()
  {
    this.SetPwmRoutineStart(0, (int) UserPanel.testDuration.TotalMilliseconds);
  }
  ...
  private void SetPwmRoutineStart(int controlValue, int controlTime)
  {
    ...
    this.AddLogLabel(string.Format((IFormatProvider) CultureInfo.CurrentCulture, Resources.MessageFormat_Setting0To1, (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 12), (object) controlValue));
    service.InputValues[0].Value = (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 12);
  ...
  private void SetSwRoutineStart(int controlTime)
  {
    ...
    this.AddLogLabel(string.Format((IFormatProvider) CultureInfo.CurrentCulture, Resources.MessageFormat_Setting0, (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 8)));
    service.InputValues[0].Value = (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 8);
  ...
  private void OnRepeatSwRoutineTimerTick(object sender, EventArgs args)
  {
    ...
    if (service != (Service) null)
    {
      ...
      this.StopTest(Resources.Message_CancellingTheTestUnableToKeepFuelCutoffValveOpen);
  }
  ...
  private void StopFuelCutOffValve() => this.SetSwRoutineStop();

  private void StopHCDoserValve() => this.SetPwmRoutineStop();

  private void SetPwmRoutineStop()
  {
    Service service = this.GetService("MCM", "RT_SR003_PWM_Routine_for_Production_Stop");
    this.AddLogLabel(string.Format((IFormatProvider) CultureInfo.CurrentCulture, Resources.MessageFormat_Resetting0, (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 12)));
    service.InputValues[0].Value = (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 12);
  ...
  private void SetSwRoutineStop()
  {
    ...
    this.AddLogLabel(string.Format((IFormatProvider) CultureInfo.CurrentCulture, Resources.MessageFormat_Resetting01, (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 8)));
    service.InputValues[0].Value = (object) service.InputValues[0].Choices.GetItemFromRawValue((object) 8);
    service.ServiceCompleteEvent += new ServiceCompleteEventHandler(this.OnPwmRoutineStopServiceCompleteEvent);
    service.Execute(false);
  }
  ...
  private void InitializeComponent()
  {
    ...
    ((Collection<Qualifier>) this.chartInstrument.Instruments).Add(new Qualifier((QualifierTypes) 1, "MCM", "DT_AS024_Fuel_Compensation_Pressure"));
    ...
    ((SingleInstrumentBase) this.fuelCompensationPressureInstrument).Instrument = new Qualifier((QualifierTypes) 1, "MCM", "DT_AS024_Fuel_Compensation_Pressure");
