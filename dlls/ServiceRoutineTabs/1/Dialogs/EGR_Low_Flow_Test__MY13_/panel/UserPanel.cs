...
// Type: DetroitDiesel.ServiceRoutineTabs.Dialogs.EGR_Low_Flow_Test__MY13_.panel.UserPanel
...
public class UserPanel : CustomPanel
{
  ...
  public UserPanel()
  {
    ...
    this.idleSpeedModificationStop = SapiManager.GetCancelSetEngineSpeedService(SapiManager.GlobalInstance.ActiveChannels);
    this.idleSpeedModificationStart = SapiManager.GetSetEngineSpeedService(SapiManager.GlobalInstance.ActiveChannels, new string[0]);
  }
      ...
      else
      {
        switch (this.currentWaitState)
        {
          case UserPanel.WaitState.WaitingToInitiateRampUp:
            if (this.ManipulateIdleSpeed(1200))
            {
          ...
          case UserPanel.WaitState.WaitingToInitiateRunOffRampUp:
            if (this.ManipulateIdleSpeed(1800))
            {
      ...
      if (this.TestRunning)
      {
        switch (this.currentWaitState)
        {
          ...
          case UserPanel.WaitState.WaitingToInitiateRunOffRampUp:
            ...
            break;
          case UserPanel.WaitState.WaitingForRampUp:
            str = string.Format((IFormatProvider) CultureInfo.CurrentCulture, Resources.MessageFormat_WaitingForEngineSpeedToReach0Rpm, (object) 1200);
            break;
          case UserPanel.WaitState.WaitingForThermalCondition:
          ...
          case UserPanel.WaitState.WaitingForEGRInRange:
            ...
            break;
          case UserPanel.WaitState.WaitingForRunOffRampUp:
            str = string.Format((IFormatProvider) CultureInfo.CurrentCulture, Resources.MessageFormat_WaitingForEngineSpeedToReach0Rpm1, (object) 1800);
            break;
          case UserPanel.WaitState.WaitingForRunOffPeriod:
    ...
    else
    {
      switch (this.currentState)
      {
        ...
        case UserPanel.State.RequestIdleModificationStop:
          ...
          Service service = ((ServiceCall) ref this.idleSpeedModificationStop).GetService();
          if (service != (Service) null)
          {
            service.ServiceCompleteEvent += new ServiceCompleteEventHandler(this.OnIdleSpeedStopServiceComplete);
  ...
  private bool ManipulateIdleSpeed(int speed)
  {
    ...
    Service service = ((ServiceCall) ref this.idleSpeedModificationStart).GetService();
    if (service != (Service) null)
    {
      service.ServiceCompleteEvent += new ServiceCompleteEventHandler(this.OnIdleSpeedStartServiceComplete);
  ...
  {
    (sender as Service).ServiceCompleteEvent -= new ServiceCompleteEventHandler(this.OnIdleSpeedStartServiceComplete);
    if (e.Succeeded)
      return;
  ...
  {
    (sender as Service).ServiceCompleteEvent -= new ServiceCompleteEventHandler(this.OnIdleSpeedStopServiceComplete);
    if (!e.Succeeded)
      this.Output(e.Exception.Message);
    this.GoMachine();
  }
  ...
  private void InitializeComponent()
  {
    ...
    ((SingleInstrumentBase) this.barInstrumentEGRDeltaPressure).Instrument = new Qualifier((QualifierTypes) 1, "MCM21T", "DT_AS025_EGR_Delta_Pressure");
    ...
    ((SingleInstrumentBase) this.barInstrumentEGRCommandedValue).Instrument = new Qualifier((QualifierTypes) 1, "MCM21T", "DT_AS031_EGR_Commanded_Governor_Value");
    ...
    ((SingleInstrumentBase) this.barInstrumentChargeAirCoolerOutTemp).Instrument = new Qualifier((QualifierTypes) 1, "MCM21T", "DT_AS060_Charge_Air_Cooler_Outlet_Temperature");
    ...
    ((SingleInstrumentBase) this.digitalReadoutFault).Instrument = new Qualifier((QualifierTypes) 32 /*0x20*/, "MCM21T", "630A12");
    ...
    ((SingleInstrumentBase) this.digitalReadoutEngineSpeed).Instrument = new Qualifier((QualifierTypes) 1, "MCM21T", "DT_AS010_Engine_Speed");
