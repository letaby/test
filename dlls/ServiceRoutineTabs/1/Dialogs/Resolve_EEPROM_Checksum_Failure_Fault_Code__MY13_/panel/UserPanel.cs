...
// Type: DetroitDiesel.ServiceRoutineTabs.Dialogs.Resolve_EEPROM_Checksum_Failure_Fault_Code__MY13_.panel.UserPanel
...
public class UserPanel : CustomPanel
{
  ...
  public virtual void OnChannelsChanged() => this.SetCPC(this.GetChannel("CPC04T"));
    ...
    if (e.Succeeded)
    {
      ...
      switch (this.CurrentStage)
      {
        case UserPanel.Stage.ReadParameters:
          ...
          foreach (Parameter parameter in (ReadOnlyCollection<Parameter>) this.cpc.Parameters)
  ...
  private void OnServiceComplete(object sender, ResultEventArgs e)
  {
    ...
    this.cpc.Services.ServiceCompleteEvent -= new ServiceCompleteEventHandler(this.OnServiceComplete);
    if (e.Succeeded)
    {
  ...
  private bool FaultIsPresent
  {
    get
    {
      if (this.Online && this.cpc.FaultCodes.HaveBeenReadFromEcu)
      {
        FaultCode faultCode = this.cpc.FaultCodes["740202"];
        if (faultCode != null)
        {
  ...
  private void UpdateUserInterface()
  {
    ...
    else
    {
      ...
      this.labelDTC.Text = Resources.Message_TheCPC04TIsNotOnlineSoFaultCodesCouldNotBeRead;
    }
  }
  ...
  private void SetCPC(Channel channel)
  {
    ...
    if (this.cpc != null)
    {
      ...
      this.cpc.FaultCodes.FaultCodesUpdateEvent -= new FaultCodesUpdateEventHandler(this.OnFaultCodesUpdate);
  ...
  {
    if (this.cpc != null && this.cpc.Online)
    {
      ...
      switch (this.CurrentStage)
      {
        case UserPanel.Stage.ReadParameters:
          ...
          this.cpc.Parameters.ParametersReadCompleteEvent += new ParametersReadCompleteEventHandler(this.OnParametersReadComplete);
          this.cpc.Parameters.ReadAll(false);
          break;
        case UserPanel.Stage.Reset:
          ...
          this.ExecuteService("RT_Reload_Original_CPC_Factory_Settings_Start_Routine_Status;FN_KeyOffOnReset");
          break;
        case UserPanel.Stage.ReadResetParameters:
        ...
        case UserPanel.Stage.WriteParameters:
          ...
          if (num > 0)
          {
            ...
            this.cpc.EcuInfos["CO_VIN"].Read(true);
  ...
  private int AssignParameters()
  {
    ...
    {
      Parameter parameter2 = this.cpc.Parameters[parameter1.Key];
  ...
  {
    this.cpc.Services.ServiceCompleteEvent += new ServiceCompleteEventHandler(this.OnServiceComplete);
    if (this.cpc.Services.Execute("RT_Reload_Original_CPC_Factory_Settings_Start_Routine_Status;FN_KeyOffOnReset", false) != 0)
      return;
    this.cpc.Services.ServiceCompleteEvent -= new ServiceCompleteEventHandler(this.OnServiceComplete);
  ...
  private bool Unlock()
  {
    try
    {
      this.cpc.Extension.Invoke(nameof (Unlock), (object[]) null);
      return true;
    }
      ...
      case UserPanel.Reason.NoParametersChanged:
        ...
        switch (reason - 1)
        {
          ...
          case UserPanel.Reason.Closing:
            this.ReportResult(Resources.Message_TheCPC04TWasDisconnected);
            break;
