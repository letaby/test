...
// Type: DetroitDiesel.ServiceRoutineTabs.Dialogs.Real_time_Clock__MY13_.panel.UserPanel
...
public class UserPanel : CustomPanel
{
  ...
  private void SetCPC(Channel cpc)
  {
    ...
    {
      this.cpc.CommunicationsStateUpdateEvent += new CommunicationsStateUpdateEventHandler(this.OnChannelStateUpdate);
      this.ecuInfo = this.cpc.EcuInfos["CO_RealTimeClock"];
      if (this.ecuInfo != null)
        this.ecuInfo.EcuInfoUpdateEvent += new EcuInfoUpdateEventHandler(this.OnEcuTimeUpdate);
    }
  }
  ...
  private void SetRealTimeClock(DateTime dateTime)
  {
    if (!this.CanSetClock)
      return;
    this.ClearResults();
    this.service = this.GetService("CPC04T", "DL_ID_Real_Time_Clock");
    if (this.service != (Service) null)
    {
      ...
      this.ReportResult(string.Format(Resources.MessageFormat_SettingRealTimeClockTo, (object) dateTime.ToString("yyyy/MM/dd HH:mm:ss"), (object) str, (object) universalTime.ToString("yyyy/MM/dd HH:mm:ss")));
  ...
  private void OnServiceComplete(object sender, ResultEventArgs e)
  {
    ...
    if (e.Succeeded)
      ...
      this.ReportResult(string.Format(Resources.MessageFormat_AnErrorOccurredWhileSettingTheRealTimeClock01PleaseCycleTheIgnitionAndTryAgain1IfTheEngineIsRunningPleaseStopTheEngineBeforeRetrying, (object) e.Exception.Message, (object) Environment.NewLine));
