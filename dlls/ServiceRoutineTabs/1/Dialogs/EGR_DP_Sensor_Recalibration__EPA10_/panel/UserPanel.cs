...
// Type: DetroitDiesel.ServiceRoutineTabs.Dialogs.EGR_DP_Sensor_Recalibration__EPA10_.panel.UserPanel
...
public class UserPanel : CustomPanel
{
  ...
  {
    bool flag = false;
    string str = Resources.Message_MCM2IsNotConnected;
    if (this.mcm != null)
    {
      if (this.mcm.CommunicationsState == CommunicationsState.Online)
      {
        str = Resources.Message_MCM2IsConnected;
        flag = true;
      }
      else
        str = Resources.Message_MCM2IsBusy;
    }
  ...
  private void OnServiceComplete(object sender, ResultEventArgs e)
  {
    this.recalibrationService.ServiceCompleteEvent -= new ServiceCompleteEventHandler(this.OnServiceComplete);
    if (e.Succeeded)
      this.ReportResult(string.Format(Resources.MessageFormat_RecalibrationExecuted, (object) this.recalibrationService.OutputValues[0].Value.ToString()));
    else
      this.ReportResult(string.Format(Resources.MessageFormat_AnErrorOccurredDuringRecalibration0, (object) e.Exception.Message));
