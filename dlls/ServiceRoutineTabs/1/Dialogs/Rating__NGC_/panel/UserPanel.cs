...
// Type: DetroitDiesel.ServiceRoutineTabs.Dialogs.Rating__NGC_.panel.UserPanel
...
#nullable disable
namespace DetroitDiesel.ServiceRoutineTabs.Dialogs.Rating__NGC_.panel;

public class UserPanel : CustomPanel
{
  private const int HighPower = 0;
  private const int LowPower = 1;
  private const int CriusePower = 3;
  private const string CruisePowerParameterName = "ptconf_p_Veh_FlcSelPar_u8";
  ...
  private const string CPCName = "CPC302T";
  ...
  {
    if (!(this.SetCPC(this.GetChannel("CPC302T", (CustomPanel.ChannelLookupOptions) 5)) | this.SetMCM(this.GetChannel("MCM21T"))))
      return;
  ...
  private bool SetCPC(Channel cpc)
  {
    ...
    if (this.cpc != cpc)
    {
      ...
      if (this.cpc != null)
      {
        ...
        this.cpc.Parameters.ParametersWriteCompleteEvent -= new ParametersWriteCompleteEventHandler(this.OnParametersWriteComplete);
      ...
      {
        this.cpc.CommunicationsStateUpdateEvent += new CommunicationsStateUpdateEventHandler(this.OnChannelStateUpdate);
        this.cruisePowerParameter = this.cpc.Parameters["ptconf_p_Veh_FlcSelPar_u8"];
        this.cruisePowerParameter.ParameterUpdateEvent += new ParameterUpdateEventHandler(this.OnCruisePowerParameterUpdate);
        this.cpc.Parameters.ParametersWriteCompleteEvent += new ParametersWriteCompleteEventHandler(this.OnParametersWriteComplete);
        this.cpc.Parameters.ParametersReadCompleteEvent += new ParametersReadCompleteEventHandler(this.OnParametersReadComplete);
        if (this.CanRead)
  ...
  private static bool HasUnsentChanges(Channel channel)
  {
    ...
    {
      if ((parameter.Qualifier != "ptconf_p_Veh_FlcSelPar_u8" || !string.Equals(channel.Ecu.Name, "CPC302T", StringComparison.OrdinalIgnoreCase) && !string.Equals(channel.Ecu.Name, "CPC501T", StringComparison.OrdinalIgnoreCase)) && !object.Equals(parameter.Value, parameter.OriginalValue))
      {
        flag = true;
        break;
      }
  ...
  private bool Busy => this.Online && this.cpc.CommunicationsState != CommunicationsState.Online;

  private bool CanRead => this.Online && !this.Busy && !this.HaveReadCruisePower;

  private bool CanWrite
  {
    get
    {
      return this.Online && !this.Busy && this.HaveReadCruisePower && !object.Equals(this.cruisePowerParameter.Value, this.cruisePowerParameter.OriginalValue);
    }
  }

  private bool CanClose => !this.Busy;
  ...
  private bool HaveReadDescriptions
  {
    get
    {
      return this.mcm != null && this.mcm.Online && this.mcm.CommunicationsState != CommunicationsState.ReadEcuInfo && this.mcm.CommunicationsState != CommunicationsState.OnlineButNotInitialized;
    }
  }
  ...
  private void UpdateUserInterface()
  {
    ...
    bool flag3 = this.HaveReadCruisePower && this.cruisePowerParameter.Choices.Count > 3 && !this.Busy;
  ...
  private void OnParametersWriteComplete(object sender, ResultEventArgs e)
  {
    if (e.Succeeded)
    {
      this.ReportResult(Resources.Message_SettingSuccessfullySent1);
  ...
  private void ReadRating()
  {
    if (!this.CanRead)
      return;
    this.ClearResults();
    this.ReportResult(Resources.Message_ReadingSettingFromCPC);
    this.cpc.Parameters.ReadGroup(this.cruisePowerParameter.GroupQualifier, true, false);
  }
  ...
  private void OnWriteRatingClick(object sender, EventArgs e)
  {
    if (!this.CanWrite)
      return;
    this.ClearResults();
    if (this.Unlock(this.cpc))
    {
      this.ReportResult(Resources.Message_SendingSettingToCPC);
  ...
  {
    if (this.CanRead && !this.HaveReadCruisePower)
      this.ReadRating();
    else
      this.UpdateUserInterface();
  }
  ...
  private void OnCruisePowerClick(object sender, EventArgs e)
  {
    if (this.cruisePowerParameter == null)
      return;
    this.cruisePowerParameter.Value = (object) this.cruisePowerParameter.Choices[3];
    this.UpdateUserInterface();
  }
