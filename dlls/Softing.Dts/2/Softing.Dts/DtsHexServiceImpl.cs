...
internal class DtsHexServiceImpl : MappedObject, DtsHexService, MCDHexService, MCDDiagService, MCDDataPrimitive, MCDDiagComPrimitive, MCDObject, IDisposable, DtsDiagService, DtsDataPrimitive, DtsDiagComPrimitive, DtsObject
{
	...
	public uint ServiceID
	{
		get
		{
			GC.KeepAlive(this);
			uint returnValue;
			IntPtr intPtr = CSWrap.CSNIDTS_DtsHexService_getServiceID(Handle, out returnValue);
		...
		{
			GC.KeepAlive(this);
			IntPtr intPtr = CSWrap.CSNIDTS_DtsHexService_setServiceID(Handle, value);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
	...
	public DtsHexServiceImpl(IntPtr handle)
	{
		Handle = handle;
