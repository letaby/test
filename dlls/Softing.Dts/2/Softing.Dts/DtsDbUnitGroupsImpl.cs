...
internal class DtsDbUnitGroupsImpl : MappedObject, DtsDbUnitGroups, MCDDbUnitGroups, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	private class DtsDbUnitGroupsEnumerator : IEnumerator
	{
		private DtsDbUnitGroupsImpl m_classList;

		private int m_index;

		public object Current => m_classList.GetItemByIndex((uint)m_index);

		public DtsDbUnitGroupsEnumerator(DtsDbUnitGroupsImpl classList)
		{
	...
	public DtsDbUnitGroupsImpl(IntPtr handle)
	{
		Handle = handle;
	...
	public MCDDbUnitGroup GetItemByIndex(uint index)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbUnitGroups_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDDbUnitGroup GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbUnitGroups_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public List<MCDDbUnitGroup> ToList()
	{
		List<MCDDbUnitGroup> list = new List<MCDDbUnitGroup>();
	...
	public MCDDbUnitGroup[] ToArray()
	{
		MCDDbUnitGroup[] array = new MCDDbUnitGroup[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbUnitGroupsEnumerator(this);
	}
}
