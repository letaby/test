...
internal class DtsDbLogicalLinksImpl : MappedObject, DtsDbLogicalLinks, MCDDbLogicalLinks, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	private class DtsDbLogicalLinksEnumerator : IEnumerator
	{
		private DtsDbLogicalLinksImpl m_classList;

		private int m_index;

		public object Current => m_classList.GetItemByIndex((uint)m_index);

		public DtsDbLogicalLinksEnumerator(DtsDbLogicalLinksImpl classList)
		{
	...
	public DtsDbLogicalLinksImpl(IntPtr handle)
	{
		Handle = handle;
	...
	~DtsDbLogicalLinksImpl()
	{
		Dispose(disposing: false);
	}

	public MCDDbLogicalLink GetItemByIndex(uint index)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbLogicalLinks_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDDbLogicalLink GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbLogicalLinks_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public List<MCDDbLogicalLink> ToList()
	{
		List<MCDDbLogicalLink> list = new List<MCDDbLogicalLink>();
	...
	public MCDDbLogicalLink[] ToArray()
	{
		MCDDbLogicalLink[] array = new MCDDbLogicalLink[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbLogicalLinksEnumerator(this);
	}
}
