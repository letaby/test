...
internal class DtsDbFlashSessionClassesImpl : MappedObject, DtsDbFlashSessionClasses, MCDDbFlashSessionClasses, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	...
	public MCDDbFlashSessionClass GetItemByIndex(uint index)
	{
		...
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbFlashSessionClasses_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
		return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFlashSessionClass;
	}
	...
	public MCDDbFlashSessionClass GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbFlashSessionClasses_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		MCDDbFlashSessionClass[] array = new MCDDbFlashSessionClass[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbFlashSessionClassesEnumerator(this);
	}
}
