...
internal class DtsAccessKeysImpl : MappedObject, DtsAccessKeys, MCDAccessKeys, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsCollection, DtsObject
{
	...
	public MCDAccessKey GetItemByIndex(uint index)
	{
		...
		IntPtr intPtr = CSWrap.CSNIDTS_DtsAccessKeys_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		MCDAccessKey[] array = new MCDAccessKey[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsAccessKeysEnumerator(this);
	}
}
