...
internal class DtsInterfaceInformationsImpl : MappedObject, DtsInterfaceInformations, DtsCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsObject
{
	...
	public DtsInterfaceInformation GetItemByIndex(uint index)
	{
		...
		IntPtr intPtr = CSWrap.CSNIDTS_DtsInterfaceInformations_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		DtsInterfaceInformation[] array = new DtsInterfaceInformation[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsInterfaceInformationsEnumerator(this);
	}
}
