...
internal class DtsRawServiceImpl : MappedObject, DtsRawService, DtsDiagService, MCDDiagService, MCDDataPrimitive, MCDDiagComPrimitive, MCDObject, IDisposable, DtsDataPrimitive, DtsDiagComPrimitive, DtsObject
{
	...
	public MCDValue RawData
	{
		get
		{
			...
			IntPtr intPtr = CSWrap.CSNIDTS_DtsRawService_getRawData(Handle, out returnValue);
	...
	{
		GC.KeepAlive(this);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsRawService_enterRawData(Handle, DTS_ObjectMapper.getHandle(rawData as MappedObject));
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
