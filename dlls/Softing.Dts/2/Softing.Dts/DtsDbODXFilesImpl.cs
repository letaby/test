...
internal class DtsDbODXFilesImpl : MappedObject, DtsDbODXFiles, DtsNamedCollection, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsCollection, DtsObject
{
	private class DtsDbODXFilesEnumerator : IEnumerator
	{
		private DtsDbODXFilesImpl m_classList;

		private int m_index;

		public object Current => m_classList.GetItemByIndex((uint)m_index);

		public DtsDbODXFilesEnumerator(DtsDbODXFilesImpl classList)
		{
	...
	public DtsDbODXFilesImpl(IntPtr handle)
	{
		Handle = handle;
	...
	public DtsDbODXFile GetItemByIndex(uint index)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbODXFiles_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
		return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbODXFile;
	}
	...
	public List<DtsDbODXFile> ToList()
	{
		List<DtsDbODXFile> list = new List<DtsDbODXFile>();
	...
	public DtsDbODXFile[] ToArray()
	{
		DtsDbODXFile[] array = new DtsDbODXFile[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbODXFilesEnumerator(this);
	}
}
