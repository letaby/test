...
internal class DtsDbJobsImpl : MappedObject, DtsDbJobs, MCDDbJobs, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	...
	public MCDDbJob GetItemByIndex(uint index)
	{
		...
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbJobs_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
		return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbJob;
	}
	...
	public MCDDbJob GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbJobs_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		MCDDbJob[] array = new MCDDbJob[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbJobsEnumerator(this);
	}
}
