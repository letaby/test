...
internal class DtsDbFunctionInParameterImpl : MappedObject, DtsDbFunctionInParameter, MCDDbFunctionInParameter, MCDDbObject, MCDNamedObject, MCDObject, IDisposable, DtsDbObject, DtsNamedObject, DtsObject
{
	...
	public MCDDataType DataType
	{
		get
		{
			...
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbFunctionInParameter_getDataType(Handle, out returnValue);
	...
	public MCDDbFunctionDiagComConnector DbFunctionDiagComConnector
	{
		get
		{
			...
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbFunctionInParameter_getDbFunctionDiagComConnector(Handle, out returnValue);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
	...
	public MCDDbRequestParameter DbRequestParameter
	{
		get
		{
			GC.KeepAlive(this);
			ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbFunctionInParameter_getDbRequestParameter(Handle, out returnValue);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
	...
	public MCDDbUnit DbUnit
	{
		get
		{
			GC.KeepAlive(this);
			ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbFunctionInParameter_getDbUnit(Handle, out returnValue);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
