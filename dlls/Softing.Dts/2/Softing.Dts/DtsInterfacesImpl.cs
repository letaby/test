...
internal class DtsInterfacesImpl : MappedObject, DtsInterfaces, MCDInterfaces, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	private class DtsInterfacesEnumerator : IEnumerator
	{
		private DtsInterfacesImpl m_classList;

		private int m_index;

		public object Current => m_classList.GetItemByIndex((uint)m_index);

		public DtsInterfacesEnumerator(DtsInterfacesImpl classList)
		{
	...
	public DtsInterfacesImpl(IntPtr handle)
	{
		Handle = handle;
	...
	public MCDInterface GetItemByIndex(uint index)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsInterfaces_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDInterface GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsInterfaces_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public List<MCDInterface> ToList()
	{
		List<MCDInterface> list = new List<MCDInterface>();
	...
	public MCDInterface[] ToArray()
	{
		MCDInterface[] array = new MCDInterface[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsInterfacesEnumerator(this);
	}
}
