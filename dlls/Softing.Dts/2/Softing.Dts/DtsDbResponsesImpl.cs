...
internal class DtsDbResponsesImpl : MappedObject, DtsDbResponses, MCDDbResponses, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	private class DtsDbResponsesEnumerator : IEnumerator
	{
		private DtsDbResponsesImpl m_classList;

		private int m_index;

		public object Current => m_classList.GetItemByIndex((uint)m_index);

		public DtsDbResponsesEnumerator(DtsDbResponsesImpl classList)
		{
	...
	public DtsDbResponsesImpl(IntPtr handle)
	{
		Handle = handle;
	...
	public MCDDbResponse GetItemByIndex(uint index)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbResponses_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDDbResponse GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbResponses_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public List<MCDDbResponse> ToList()
	{
		List<MCDDbResponse> list = new List<MCDDbResponse>();
	...
	public MCDDbResponse[] ToArray()
	{
		MCDDbResponse[] array = new MCDDbResponse[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbResponsesEnumerator(this);
	}
}
