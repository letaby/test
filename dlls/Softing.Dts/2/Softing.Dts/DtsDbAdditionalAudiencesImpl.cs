...
internal class DtsDbAdditionalAudiencesImpl : MappedObject, DtsDbAdditionalAudiences, MCDDbAdditionalAudiences, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	...
	public MCDDbAdditionalAudience GetItemByIndex(uint uIndex)
	{
		...
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbAdditionalAudiences_getItemByIndex(Handle, uIndex, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
		return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbAdditionalAudience;
	}
	...
	public MCDDbAdditionalAudience GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbAdditionalAudiences_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		MCDDbAdditionalAudience[] array = new MCDDbAdditionalAudience[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbAdditionalAudiencesEnumerator(this);
	}
}
