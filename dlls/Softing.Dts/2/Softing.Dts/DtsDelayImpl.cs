...
internal class DtsDelayImpl : MappedObject, DtsDelay, DtsControlPrimitive, MCDControlPrimitive, MCDDiagComPrimitive, MCDObject, IDisposable, DtsDiagComPrimitive, DtsObject
{
	...
	public uint TimeDelay
	{
		get
		{
			GC.KeepAlive(this);
			uint returnValue;
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDelay_getTimeDelay(Handle, out returnValue);
		...
		{
			GC.KeepAlive(this);
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDelay_setTimeDelay(Handle, value);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
