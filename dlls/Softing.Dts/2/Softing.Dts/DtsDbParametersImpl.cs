...
internal class DtsDbParametersImpl : MappedObject, DtsDbParameters, MCDDbParameters, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	private class DtsDbParametersEnumerator : IEnumerator
	{
		private DtsDbParametersImpl m_classList;

		private int m_index;

		public object Current => m_classList.GetItemByIndex((uint)m_index);

		public DtsDbParametersEnumerator(DtsDbParametersImpl classList)
		{
	...
	public DtsDbParametersImpl(IntPtr handle)
	{
		Handle = handle;
	...
	public MCDDbParameter GetItemByIndex(uint index)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbParameters_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDDbParameter GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbParameters_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public List<MCDDbParameter> ToList()
	{
		List<MCDDbParameter> list = new List<MCDDbParameter>();
	...
	public MCDDbParameter[] ToArray()
	{
		MCDDbParameter[] array = new MCDDbParameter[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbParametersEnumerator(this);
	}
}
