...
internal class DtsDbAccessLevelImpl : MappedObject, DtsDbAccessLevel, MCDDbAccessLevel, MCDDbObject, MCDNamedObject, MCDObject, IDisposable, DtsDbObject, DtsNamedObject, DtsObject
{
	...
	public MCDDbDiagComPrimitive AccessDiagComPrimitive
	{
		get
		{
			...
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbAccessLevel_getAccessDiagComPrimitive(Handle, out returnValue);
	...
	public uint AccessLevelValue
	{
		get
		{
			GC.KeepAlive(this);
			uint returnValue;
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbAccessLevel_getAccessLevelValue(Handle, out returnValue);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
	...
	public string ExternalAccessMethod
	{
		get
		{
			...
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbAccessLevel_getExternalAccessMethod(Handle, out returnValue);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
	...
	public bool HasExternalAccessMethod
	{
		get
		{
			GC.KeepAlive(this);
			bool returnValue;
			IntPtr intPtr = CSWrap.CSNIDTS_DtsDbAccessLevel_hasExternalAccessMethod(Handle, out returnValue);
			if (intPtr != IntPtr.Zero)
			{
				throw DTS_ObjectMapper.createException(intPtr);
			}
