...
internal class DtsDbControlPrimitivesImpl : MappedObject, DtsDbControlPrimitives, MCDDbControlPrimitives, MCDNamedCollection, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsNamedCollection, DtsCollection, DtsObject
{
	...
	public MCDDbControlPrimitive GetItemByIndex(uint index)
	{
		...
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbControlPrimitives_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
		return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbControlPrimitive;
	}
	...
	public MCDDbControlPrimitive GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbControlPrimitives_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDDbControlPrimitive GetItemByType(MCDObjectType type)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsDbControlPrimitives_getItemByType(Handle, type, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		MCDDbControlPrimitive[] array = new MCDDbControlPrimitive[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsDbControlPrimitivesEnumerator(this);
	}
}
