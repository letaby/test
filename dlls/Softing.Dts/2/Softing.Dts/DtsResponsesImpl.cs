...
internal class DtsResponsesImpl : MappedObject, DtsResponses, MCDResponses, MCDCollection, MCDObject, IDisposable, IEnumerable, DtsCollection, DtsObject
{
	...
	public MCDObject Parent
	{
		get
		{
			...
			IntPtr intPtr = CSWrap.CSNIDTS_DtsResponses_getParent(Handle, out returnValue);
	...
	public MCDResponse GetItemByIndex(uint index)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsResponses_getItemByIndex(Handle, index, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDResponse Add(MCDDbLocation pDbLocation, bool isPositive)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsResponses_add(Handle, DTS_ObjectMapper.getHandle(pDbLocation as MappedObject), isPositive, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		GC.KeepAlive(this);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsResponses_removeByIndex(Handle, index);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	public MCDResponse GetItemByName(string name)
	{
		GC.KeepAlive(this);
		ObjectInfo_Struct returnValue = default(ObjectInfo_Struct);
		IntPtr intPtr = CSWrap.CSNIDTS_DtsResponses_getItemByName(Handle, name, out returnValue);
		if (intPtr != IntPtr.Zero)
		{
			throw DTS_ObjectMapper.createException(intPtr);
		}
	...
	{
		MCDResponse[] array = new MCDResponse[Count];
		for (uint num = 0u; num < Count; num++)
		{
	...
	{
		return new DtsResponsesEnumerator(this);
	}
}
