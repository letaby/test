...
// Type: Softing.Dts.DtsDbVehicleConnectorPinsImpl
...
internal class DtsDbVehicleConnectorPinsImpl : 
  MappedObject,
  DtsDbVehicleConnectorPins,
  ...
  DtsObject
{
  ...
  public DtsDbVehicleConnectorPinsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbVehicleConnectorPinsImpl() => this.Dispose(false);
  ...
  public MCDDbVehicleConnectorPin GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbVehicleConnectorPins_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbVehicleConnectorPin) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbVehicleConnectorPin);
  }

  public MCDDbVehicleConnectorPin GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbVehicleConnectorPins_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbVehicleConnectorPin> ToList()
  {
    List<MCDDbVehicleConnectorPin> list = new List<MCDDbVehicleConnectorPin>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbVehicleConnectorPin[] ToArray()
  {
    MCDDbVehicleConnectorPin[] array = new MCDDbVehicleConnectorPin[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbVehicleConnectorPinsImpl.DtsDbVehicleConnectorPinsEnumerator(this);
  }

  private class DtsDbVehicleConnectorPinsEnumerator : IEnumerator
  {
    private DtsDbVehicleConnectorPinsImpl m_classList;
    private int m_index;

    public DtsDbVehicleConnectorPinsEnumerator(DtsDbVehicleConnectorPinsImpl classList)
    {
