...
// Type: Softing.Dts.DtsDbComponentConnectorsImpl
...
internal class DtsDbComponentConnectorsImpl : 
  MappedObject,
  DtsDbComponentConnectors,
  ...
  DtsObject
{
  ...
  ~DtsDbComponentConnectorsImpl() => this.Dispose(false);
  ...
  public MCDDbComponentConnector GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbComponentConnectors_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbComponentConnector) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbComponentConnector);
  }
  ...
  {
    MCDDbComponentConnector[] array = new MCDDbComponentConnector[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbComponentConnectorsImpl.DtsDbComponentConnectorsEnumerator(this);
  }
