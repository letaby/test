...
// Type: Softing.Dts.DtsDbFunctionNodeGroupsImpl
...
internal class DtsDbFunctionNodeGroupsImpl : 
  MappedObject,
  DtsDbFunctionNodeGroups,
  ...
  DtsObject
{
  ...
  public DtsDbFunctionNodeGroupsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbFunctionNodeGroupsImpl() => this.Dispose(false);
  ...
  public MCDDbFunctionNodeGroup GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFunctionNodeGroups_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFunctionNodeGroup) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFunctionNodeGroup);
  }

  public MCDDbFunctionNodeGroup GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFunctionNodeGroups_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbFunctionNodeGroup> ToList()
  {
    List<MCDDbFunctionNodeGroup> list = new List<MCDDbFunctionNodeGroup>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbFunctionNodeGroup[] ToArray()
  {
    MCDDbFunctionNodeGroup[] array = new MCDDbFunctionNodeGroup[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFunctionNodeGroupsImpl.DtsDbFunctionNodeGroupsEnumerator(this);
  }

  private class DtsDbFunctionNodeGroupsEnumerator : IEnumerator
  {
    private DtsDbFunctionNodeGroupsImpl m_classList;
    private int m_index;

    public DtsDbFunctionNodeGroupsEnumerator(DtsDbFunctionNodeGroupsImpl classList)
    {
