...
// Type: Softing.Dts.DtsDbCompuScalesImpl
...
internal class DtsDbCompuScalesImpl : 
  MappedObject,
  DtsDbCompuScales,
  DtsCollection,
  MCDCollection,
  ...
  DtsObject
{
  ...
  public DtsDbCompuScalesImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbCompuScalesImpl() => this.Dispose(false);
  ...
  public DtsDbCompuScale GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbCompuScales_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbCompuScale;
  }
  ...
  public List<DtsDbCompuScale> ToList()
  {
    List<DtsDbCompuScale> list = new List<DtsDbCompuScale>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public DtsDbCompuScale[] ToArray()
  {
    DtsDbCompuScale[] array = new DtsDbCompuScale[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbCompuScalesImpl.DtsDbCompuScalesEnumerator(this);
  }

  private class DtsDbCompuScalesEnumerator : IEnumerator
  {
    private DtsDbCompuScalesImpl m_classList;
    private int m_index;

    public DtsDbCompuScalesEnumerator(DtsDbCompuScalesImpl classList)
    {
