...
// Type: Softing.Dts.DtsDbSystemItemsImpl
...
internal class DtsDbSystemItemsImpl : 
  MappedObject,
  DtsDbSystemItems,
  MCDDbSystemItems,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbSystemItemsImpl() => this.Dispose(false);
  ...
  public MCDDbSystemItem GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbSystemItems_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbSystemItem) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbSystemItem);
  }
  ...
  public MCDDbSystemItem GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbSystemItems_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbSystemItem[] array = new MCDDbSystemItem[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbSystemItemsImpl.DtsDbSystemItemsEnumerator(this);
  }
