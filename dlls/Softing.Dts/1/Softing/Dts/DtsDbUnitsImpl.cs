...
// Type: Softing.Dts.DtsDbUnitsImpl
  ...
  DtsObject
{
  ...
  public DtsDbUnitsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbUnitsImpl() => this.Dispose(false);
  ...
  public MCDDbUnit GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbUnits_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public MCDDbUnit GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbUnits_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbUnit> ToList()
  {
    List<MCDDbUnit> list = new List<MCDDbUnit>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbUnit[] ToArray()
  {
    MCDDbUnit[] array = new MCDDbUnit[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
      array[(int) index] = this.GetItemByIndex(index);
    return array;
  }

  public IEnumerator GetEnumerator() => (IEnumerator) new DtsDbUnitsImpl.DtsDbUnitsEnumerator(this);

  private class DtsDbUnitsEnumerator : IEnumerator
  {
    private DtsDbUnitsImpl m_classList;
    private int m_index;

    public DtsDbUnitsEnumerator(DtsDbUnitsImpl classList)
    {
