...
// Type: Softing.Dts.DtsDbFlashSegmentsImpl
...
internal class DtsDbFlashSegmentsImpl : 
  MappedObject,
  DtsDbFlashSegments,
  MCDDbFlashSegments,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbFlashSegmentsImpl() => this.Dispose(false);
  ...
  public MCDDbFlashSegment GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFlashSegments_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFlashSegment) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFlashSegment);
  }
  ...
  public MCDDbFlashSegment GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFlashSegments_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbFlashSegment[] array = new MCDDbFlashSegment[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFlashSegmentsImpl.DtsDbFlashSegmentsEnumerator(this);
  }
