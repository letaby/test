...
// Type: Softing.Dts.DtsDbFunctionalGroupsImpl
...
internal class DtsDbFunctionalGroupsImpl : 
  MappedObject,
  DtsDbFunctionalGroups,
  ...
  DtsObject
{
  ...
  public DtsDbFunctionalGroupsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbFunctionalGroupsImpl() => this.Dispose(false);
  ...
  public MCDDbFunctionalGroup GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFunctionalGroups_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFunctionalGroup) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFunctionalGroup);
  }

  public MCDDbFunctionalGroup GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFunctionalGroups_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbFunctionalGroup> ToList()
  {
    List<MCDDbFunctionalGroup> list = new List<MCDDbFunctionalGroup>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbFunctionalGroup[] ToArray()
  {
    MCDDbFunctionalGroup[] array = new MCDDbFunctionalGroup[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFunctionalGroupsImpl.DtsDbFunctionalGroupsEnumerator(this);
  }

  private class DtsDbFunctionalGroupsEnumerator : IEnumerator
  {
    private DtsDbFunctionalGroupsImpl m_classList;
    private int m_index;

    public DtsDbFunctionalGroupsEnumerator(DtsDbFunctionalGroupsImpl classList)
    {
