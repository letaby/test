...
// Type: Softing.Dts.DtsInterfaceConfigsImpl
...
internal class DtsInterfaceConfigsImpl : 
  MappedObject,
  DtsInterfaceConfigs,
  DtsCollection,
  MCDCollection,
  ...
  DtsObject
{
  ...
  ~DtsInterfaceConfigsImpl() => this.Dispose(false);
  ...
  public DtsInterfaceConfig GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsInterfaceConfigs_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public DtsInterfaceConfig CreateInterface()
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr objectHandle = CSWrap.CSNIDTS_DtsInterfaceConfigs_createInterface(this.Handle, out returnValue);
  ...
  {
    GC.KeepAlive((object) this);
    IntPtr objectHandle = CSWrap.CSNIDTS_DtsInterfaceConfigs_remove(this.Handle, DTS_ObjectMapper.getHandle(interfaceConfig as MappedObject));
    if (objectHandle != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(objectHandle);
  }
  ...
  {
    DtsInterfaceConfig[] array = new DtsInterfaceConfig[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsInterfaceConfigsImpl.DtsInterfaceConfigsEnumerator(this);
  }
