...
// Type: Softing.Dts.DtsDbFunctionNodesImpl
...
internal class DtsDbFunctionNodesImpl : 
  MappedObject,
  DtsDbFunctionNodes,
  MCDDbFunctionNodes,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbFunctionNodesImpl() => this.Dispose(false);
  ...
  public MCDDbFunctionNode GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFunctionNodes_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFunctionNode) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFunctionNode);
  }
  ...
  public MCDDbFunctionNode GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFunctionNodes_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbFunctionNode[] array = new MCDDbFunctionNode[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFunctionNodesImpl.DtsDbFunctionNodesEnumerator(this);
  }
