...
// Type: Softing.Dts.DtsDbEcuStatesImpl
...
internal class DtsDbEcuStatesImpl : 
  MappedObject,
  DtsDbEcuStates,
  MCDDbEcuStates,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  public DtsDbEcuStatesImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbEcuStatesImpl() => this.Dispose(false);
  ...
  public MCDDbEcuState GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbEcuStates_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public MCDDbEcuState GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbEcuStates_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbEcuState> ToList()
  {
    List<MCDDbEcuState> list = new List<MCDDbEcuState>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbEcuState[] ToArray()
  {
    MCDDbEcuState[] array = new MCDDbEcuState[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbEcuStatesImpl.DtsDbEcuStatesEnumerator(this);
  }

  private class DtsDbEcuStatesEnumerator : IEnumerator
  {
    private DtsDbEcuStatesImpl m_classList;
    private int m_index;

    public DtsDbEcuStatesEnumerator(DtsDbEcuStatesImpl classList)
    {
