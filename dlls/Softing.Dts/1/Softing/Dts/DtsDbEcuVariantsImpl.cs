...
// Type: Softing.Dts.DtsDbEcuVariantsImpl
...
internal class DtsDbEcuVariantsImpl : 
  MappedObject,
  DtsDbEcuVariants,
  MCDDbEcuVariants,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbEcuVariantsImpl() => this.Dispose(false);
  ...
  public MCDDbEcuVariant GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbEcuVariants_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbEcuVariant) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbEcuVariant);
  }
  ...
  public MCDDbEcuVariant GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbEcuVariants_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbEcuVariant[] array = new MCDDbEcuVariant[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbEcuVariantsImpl.DtsDbEcuVariantsEnumerator(this);
  }
