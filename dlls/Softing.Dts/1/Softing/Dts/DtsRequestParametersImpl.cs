...
// Type: Softing.Dts.DtsRequestParametersImpl
...
internal class DtsRequestParametersImpl : 
  MappedObject,
  DtsRequestParameters,
  ...
  DtsObject
{
  ...
  public DtsRequestParametersImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsRequestParametersImpl() => this.Dispose(false);
  ...
  public MCDRequestParameter GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsRequestParameters_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public MCDRequestParameter GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsRequestParameters_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDRequestParameter> ToList()
  {
    List<MCDRequestParameter> list = new List<MCDRequestParameter>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDRequestParameter[] ToArray()
  {
    MCDRequestParameter[] array = new MCDRequestParameter[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsRequestParametersImpl.DtsRequestParametersEnumerator(this);
  }

  private class DtsRequestParametersEnumerator : IEnumerator
  {
    private DtsRequestParametersImpl m_classList;
    private int m_index;

    public DtsRequestParametersEnumerator(DtsRequestParametersImpl classList)
    {
