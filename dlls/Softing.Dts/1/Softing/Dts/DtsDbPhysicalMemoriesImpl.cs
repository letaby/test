...
// Type: Softing.Dts.DtsDbPhysicalMemoriesImpl
...
internal class DtsDbPhysicalMemoriesImpl : 
  MappedObject,
  DtsDbPhysicalMemories,
  ...
  DtsObject
{
  ...
  public DtsDbPhysicalMemoriesImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbPhysicalMemoriesImpl() => this.Dispose(false);
  ...
  public MCDDbPhysicalMemory GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbPhysicalMemories_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbPhysicalMemory) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbPhysicalMemory);
  }

  public MCDDbPhysicalMemory GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbPhysicalMemories_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbPhysicalMemory> ToList()
  {
    List<MCDDbPhysicalMemory> list = new List<MCDDbPhysicalMemory>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbPhysicalMemory[] ToArray()
  {
    MCDDbPhysicalMemory[] array = new MCDDbPhysicalMemory[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbPhysicalMemoriesImpl.DtsDbPhysicalMemoriesEnumerator(this);
  }

  private class DtsDbPhysicalMemoriesEnumerator : IEnumerator
  {
    private DtsDbPhysicalMemoriesImpl m_classList;
    private int m_index;

    public DtsDbPhysicalMemoriesEnumerator(DtsDbPhysicalMemoriesImpl classList)
    {
