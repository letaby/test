...
// Type: Softing.Dts.DtsErrorsImpl
  ...
  DtsObject
{
  ...
  ~DtsErrorsImpl() => this.Dispose(false);
  ...
  public MCDError GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsErrors_getItemByIndex(this.Handle, index, out returnValue);
  ...
  {
    MCDError[] array = new MCDError[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
      array[(int) index] = this.GetItemByIndex(index);
    return array;
  }

  public IEnumerator GetEnumerator() => (IEnumerator) new DtsErrorsImpl.DtsErrorsEnumerator(this);
