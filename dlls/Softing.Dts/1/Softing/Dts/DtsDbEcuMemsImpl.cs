...
// Type: Softing.Dts.DtsDbEcuMemsImpl
...
internal class DtsDbEcuMemsImpl : 
  MappedObject,
  DtsDbEcuMems,
  MCDDbEcuMems,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbEcuMemsImpl() => this.Dispose(false);
  ...
  public MCDDbEcuMem GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbEcuMems_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbEcuMem) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbEcuMem);
  }
  ...
  public MCDDbEcuMem GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbEcuMems_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbEcuMem[] array = new MCDDbEcuMem[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbEcuMemsImpl.DtsDbEcuMemsEnumerator(this);
  }
