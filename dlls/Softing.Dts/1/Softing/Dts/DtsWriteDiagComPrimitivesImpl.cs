...
// Type: Softing.Dts.DtsWriteDiagComPrimitivesImpl
...
internal class DtsWriteDiagComPrimitivesImpl : 
  MappedObject,
  DtsWriteDiagComPrimitives,
  ...
  DtsObject
{
  ...
  public DtsWriteDiagComPrimitivesImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsWriteDiagComPrimitivesImpl() => this.Dispose(false);
  ...
  public MCDDiagComPrimitive GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsWriteDiagComPrimitives_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public List<MCDDiagComPrimitive> ToList()
  {
    List<MCDDiagComPrimitive> list = new List<MCDDiagComPrimitive>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDiagComPrimitive[] ToArray()
  {
    MCDDiagComPrimitive[] array = new MCDDiagComPrimitive[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsWriteDiagComPrimitivesImpl.DtsWriteDiagComPrimitivesEnumerator(this);
  }

  private class DtsWriteDiagComPrimitivesEnumerator : IEnumerator
  {
    private DtsWriteDiagComPrimitivesImpl m_classList;
    private int m_index;

    public DtsWriteDiagComPrimitivesEnumerator(DtsWriteDiagComPrimitivesImpl classList)
    {
