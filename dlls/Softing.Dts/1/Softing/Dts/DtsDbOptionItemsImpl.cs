...
// Type: Softing.Dts.DtsDbOptionItemsImpl
...
internal class DtsDbOptionItemsImpl : 
  MappedObject,
  DtsDbOptionItems,
  MCDDbOptionItems,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbOptionItemsImpl() => this.Dispose(false);
  ...
  public MCDDbOptionItem GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbOptionItems_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbOptionItem) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbOptionItem);
  }
  ...
  public MCDDbOptionItem GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbOptionItems_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbOptionItem[] array = new MCDDbOptionItem[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbOptionItemsImpl.DtsDbOptionItemsEnumerator(this);
  }
