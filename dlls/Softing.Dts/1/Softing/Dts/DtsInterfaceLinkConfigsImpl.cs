...
// Type: Softing.Dts.DtsInterfaceLinkConfigsImpl
...
internal class DtsInterfaceLinkConfigsImpl : 
  MappedObject,
  DtsInterfaceLinkConfigs,
  DtsCollection,
  MCDCollection,
  MCDObject,
  IDisposable,
  ...
  DtsObject
{
  ...
  ~DtsInterfaceLinkConfigsImpl() => this.Dispose(false);
  ...
  public DtsInterfaceLinkConfig GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsInterfaceLinkConfigs_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public DtsInterfaceLinkConfig CreateInterfaceLink()
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr interfaceLink = CSWrap.CSNIDTS_DtsInterfaceLinkConfigs_createInterfaceLink(this.Handle, out returnValue);
    if (interfaceLink != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(interfaceLink);
    return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsInterfaceLinkConfig;
  }
  ...
  {
    GC.KeepAlive((object) this);
    IntPtr objectHandle = CSWrap.CSNIDTS_DtsInterfaceLinkConfigs_remove(this.Handle, DTS_ObjectMapper.getHandle(link as MappedObject));
  ...
  {
    DtsInterfaceLinkConfig[] array = new DtsInterfaceLinkConfig[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsInterfaceLinkConfigsImpl.DtsInterfaceLinkConfigsEnumerator(this);
  }
