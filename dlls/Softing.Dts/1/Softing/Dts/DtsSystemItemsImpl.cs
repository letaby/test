...
// Type: Softing.Dts.DtsSystemItemsImpl
...
internal class DtsSystemItemsImpl : 
  MappedObject,
  DtsSystemItems,
  MCDSystemItems,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsSystemItemsImpl() => this.Dispose(false);
  ...
  public MCDSystemItem GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsSystemItems_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDSystemItem) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsSystemItem);
  }
  ...
  public MCDSystemItem GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsSystemItems_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDSystemItem[] array = new MCDSystemItem[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsSystemItemsImpl.DtsSystemItemsEnumerator(this);
  }
