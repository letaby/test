...
// Type: Softing.Dts.DtsDbPhysicalSegmentsImpl
...
internal class DtsDbPhysicalSegmentsImpl : 
  MappedObject,
  DtsDbPhysicalSegments,
  ...
  DtsObject
{
  ...
  ~DtsDbPhysicalSegmentsImpl() => this.Dispose(false);
  ...
  public MCDDbPhysicalSegment GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbPhysicalSegments_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbPhysicalSegment) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbPhysicalSegment);
  }
  ...
  public MCDDbPhysicalSegment GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbPhysicalSegments_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbPhysicalSegment[] array = new MCDDbPhysicalSegment[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbPhysicalSegmentsImpl.DtsDbPhysicalSegmentsEnumerator(this);
  }
