...
// Type: Softing.Dts.DtsDbVariantCodingDomainsImpl
...
internal class DtsDbVariantCodingDomainsImpl : 
  MappedObject,
  DtsDbVariantCodingDomains,
  DtsNamedCollection,
  MCDNamedCollection,
  MCDCollection,
  MCDObject,
  ...
  DtsObject
{
  ...
  public DtsDbVariantCodingDomainsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbVariantCodingDomainsImpl() => this.Dispose(false);
  ...
  public DtsDbVariantCodingDomain GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbVariantCodingDomains_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbVariantCodingDomain;
  }

  public DtsDbVariantCodingDomain GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbVariantCodingDomains_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<DtsDbVariantCodingDomain> ToList()
  {
    List<DtsDbVariantCodingDomain> list = new List<DtsDbVariantCodingDomain>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public DtsDbVariantCodingDomain[] ToArray()
  {
    DtsDbVariantCodingDomain[] array = new DtsDbVariantCodingDomain[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbVariantCodingDomainsImpl.DtsDbVariantCodingDomainsEnumerator(this);
  }

  private class DtsDbVariantCodingDomainsEnumerator : IEnumerator
  {
    private DtsDbVariantCodingDomainsImpl m_classList;
    private int m_index;

    public DtsDbVariantCodingDomainsEnumerator(DtsDbVariantCodingDomainsImpl classList)
    {
