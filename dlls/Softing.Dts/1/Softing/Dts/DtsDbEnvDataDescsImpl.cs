...
// Type: Softing.Dts.DtsDbEnvDataDescsImpl
...
internal class DtsDbEnvDataDescsImpl : 
  MappedObject,
  DtsDbEnvDataDescs,
  MCDDbEnvDataDescs,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbEnvDataDescsImpl() => this.Dispose(false);
  ...
  public MCDDbEnvDataDesc GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbEnvDataDescs_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbEnvDataDesc) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbEnvDataDesc);
  }
  ...
  public MCDDbEnvDataDesc GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbEnvDataDescs_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbEnvDataDesc[] array = new MCDDbEnvDataDesc[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbEnvDataDescsImpl.DtsDbEnvDataDescsEnumerator(this);
  }
