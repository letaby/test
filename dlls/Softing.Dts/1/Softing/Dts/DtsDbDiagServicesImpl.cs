...
// Type: Softing.Dts.DtsDbDiagServicesImpl
...
internal class DtsDbDiagServicesImpl : 
  MappedObject,
  DtsDbDiagServices,
  MCDDbDiagServices,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbDiagServicesImpl() => this.Dispose(false);
  ...
  public MCDDbDiagService GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbDiagServices_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbDiagService) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbDiagService);
  }
  ...
  public MCDDbDiagService GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbDiagServices_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbDiagService[] array = new MCDDbDiagService[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbDiagServicesImpl.DtsDbDiagServicesEnumerator(this);
  }
