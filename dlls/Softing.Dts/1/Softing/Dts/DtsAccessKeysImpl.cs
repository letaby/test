...
// Type: Softing.Dts.DtsAccessKeysImpl
...
internal class DtsAccessKeysImpl : 
  MappedObject,
  DtsAccessKeys,
  MCDAccessKeys,
  MCDCollection,
  ...
  DtsObject
{
  ...
  public DtsAccessKeysImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsAccessKeysImpl() => this.Dispose(false);
  ...
  public MCDAccessKey GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsAccessKeys_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public List<MCDAccessKey> ToList()
  {
    List<MCDAccessKey> list = new List<MCDAccessKey>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDAccessKey[] ToArray()
  {
    MCDAccessKey[] array = new MCDAccessKey[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsAccessKeysImpl.DtsAccessKeysEnumerator(this);
  }

  private class DtsAccessKeysEnumerator : IEnumerator
  {
    private DtsAccessKeysImpl m_classList;
    private int m_index;

    public DtsAccessKeysEnumerator(DtsAccessKeysImpl classList)
    {
