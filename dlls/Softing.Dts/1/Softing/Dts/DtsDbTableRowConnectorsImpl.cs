...
// Type: Softing.Dts.DtsDbTableRowConnectorsImpl
...
internal class DtsDbTableRowConnectorsImpl : 
  MappedObject,
  DtsDbTableRowConnectors,
  ...
  DtsObject
{
  ...
  public DtsDbTableRowConnectorsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbTableRowConnectorsImpl() => this.Dispose(false);
  ...
  public MCDDbTableRowConnector GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbTableRowConnectors_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbTableRowConnector) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbTableRowConnector);
  }

  public MCDDbTableRowConnector GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbTableRowConnectors_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbTableRowConnector> ToList()
  {
    List<MCDDbTableRowConnector> list = new List<MCDDbTableRowConnector>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbTableRowConnector[] ToArray()
  {
    MCDDbTableRowConnector[] array = new MCDDbTableRowConnector[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbTableRowConnectorsImpl.DtsDbTableRowConnectorsEnumerator(this);
  }

  private class DtsDbTableRowConnectorsEnumerator : IEnumerator
  {
    private DtsDbTableRowConnectorsImpl m_classList;
    private int m_index;

    public DtsDbTableRowConnectorsEnumerator(DtsDbTableRowConnectorsImpl classList)
    {
