...
// Type: Softing.Dts.DtsDbJobsImpl
  ...
  DtsObject
{
  ...
  public DtsDbJobsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbJobsImpl() => this.Dispose(false);
  ...
  public MCDDbJob GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbJobs_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbJob) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbJob);
  }

  public MCDDbJob GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbJobs_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbJob> ToList()
  {
    List<MCDDbJob> list = new List<MCDDbJob>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbJob[] ToArray()
  {
    MCDDbJob[] array = new MCDDbJob[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
      array[(int) index] = this.GetItemByIndex(index);
    return array;
  }

  public IEnumerator GetEnumerator() => (IEnumerator) new DtsDbJobsImpl.DtsDbJobsEnumerator(this);

  private class DtsDbJobsEnumerator : IEnumerator
  {
    private DtsDbJobsImpl m_classList;
    private int m_index;

    public DtsDbJobsEnumerator(DtsDbJobsImpl classList)
    {
