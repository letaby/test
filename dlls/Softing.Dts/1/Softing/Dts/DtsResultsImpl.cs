...
// Type: Softing.Dts.DtsResultsImpl
  ...
  DtsObject
{
  ...
  public DtsResultsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsResultsImpl() => this.Dispose(false);
  ...
  public MCDResult GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsResults_getItemByIndex(this.Handle, index, out returnValue);
  ...
  public MCDResult Add(MCDError pError, MCDResultType type)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr objectHandle = CSWrap.CSNIDTS_DtsResults_add(this.Handle, DTS_ObjectMapper.getHandle(pError as MappedObject), type, out returnValue);
    if (objectHandle != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(objectHandle);
    return (MCDResult) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsResult);
  }

  public MCDResult AddWithContent(MCDResult pResult)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr objectHandle = CSWrap.CSNIDTS_DtsResults_addWithContent(this.Handle, DTS_ObjectMapper.getHandle(pResult as MappedObject), out returnValue);
    if (objectHandle != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(objectHandle);
    return (MCDResult) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsResult);
  }
  ...
  public List<MCDResult> ToList()
  {
    List<MCDResult> list = new List<MCDResult>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDResult[] ToArray()
  {
    MCDResult[] array = new MCDResult[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
      array[(int) index] = this.GetItemByIndex(index);
    return array;
  }

  public IEnumerator GetEnumerator() => (IEnumerator) new DtsResultsImpl.DtsResultsEnumerator(this);

  private class DtsResultsEnumerator : IEnumerator
  {
    private DtsResultsImpl m_classList;
    private int m_index;

    public DtsResultsEnumerator(DtsResultsImpl classList)
    {
