...
// Type: Softing.Dts.DtsDbFaultMemoriesImpl
...
internal class DtsDbFaultMemoriesImpl : 
  MappedObject,
  DtsDbFaultMemories,
  MCDDbFaultMemories,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbFaultMemoriesImpl() => this.Dispose(false);
  ...
  public MCDDbFaultMemory GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFaultMemories_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFaultMemory) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFaultMemory);
  }
  ...
  public MCDDbFaultMemory GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFaultMemories_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbFaultMemory[] array = new MCDDbFaultMemory[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFaultMemoriesImpl.DtsDbFaultMemoriesEnumerator(this);
  }
