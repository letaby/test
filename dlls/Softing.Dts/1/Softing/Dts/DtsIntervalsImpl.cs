...
// Type: Softing.Dts.DtsIntervalsImpl
...
internal class DtsIntervalsImpl : 
  MappedObject,
  DtsIntervals,
  MCDIntervals,
  MCDCollection,
  ...
  DtsObject
{
  ...
  ~DtsIntervalsImpl() => this.Dispose(false);
  ...
  public MCDInterval GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsIntervals_getItemByIndex(this.Handle, index, out returnValue);
  ...
  {
    MCDInterval[] array = new MCDInterval[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsIntervalsImpl.DtsIntervalsEnumerator(this);
  }
