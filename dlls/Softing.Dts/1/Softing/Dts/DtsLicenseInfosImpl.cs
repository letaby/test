...
// Type: Softing.Dts.DtsLicenseInfosImpl
...
internal class DtsLicenseInfosImpl : 
  MappedObject,
  DtsLicenseInfos,
  DtsCollection,
  MCDCollection,
  ...
  DtsObject
{
  ...
  public DtsLicenseInfosImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsLicenseInfosImpl() => this.Dispose(false);
  ...
  public DtsLicenseInfo GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsLicenseInfos_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsLicenseInfo;
  }
  ...
  public List<DtsLicenseInfo> ToList()
  {
    List<DtsLicenseInfo> list = new List<DtsLicenseInfo>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public DtsLicenseInfo[] ToArray()
  {
    DtsLicenseInfo[] array = new DtsLicenseInfo[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsLicenseInfosImpl.DtsLicenseInfosEnumerator(this);
  }

  private class DtsLicenseInfosEnumerator : IEnumerator
  {
    private DtsLicenseInfosImpl m_classList;
    private int m_index;

    public DtsLicenseInfosEnumerator(DtsLicenseInfosImpl classList)
    {
