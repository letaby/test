...
// Type: Softing.Dts.DtsDbVehicleConnectorsImpl
...
internal class DtsDbVehicleConnectorsImpl : 
  MappedObject,
  DtsDbVehicleConnectors,
  ...
  DtsObject
{
  ...
  public DtsDbVehicleConnectorsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbVehicleConnectorsImpl() => this.Dispose(false);
  ...
  public MCDDbVehicleConnector GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbVehicleConnectors_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbVehicleConnector) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbVehicleConnector);
  }

  public MCDDbVehicleConnector GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbVehicleConnectors_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbVehicleConnector> ToList()
  {
    List<MCDDbVehicleConnector> list = new List<MCDDbVehicleConnector>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbVehicleConnector[] ToArray()
  {
    MCDDbVehicleConnector[] array = new MCDDbVehicleConnector[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbVehicleConnectorsImpl.DtsDbVehicleConnectorsEnumerator(this);
  }

  private class DtsDbVehicleConnectorsEnumerator : IEnumerator
  {
    private DtsDbVehicleConnectorsImpl m_classList;
    private int m_index;

    public DtsDbVehicleConnectorsEnumerator(DtsDbVehicleConnectorsImpl classList)
    {
