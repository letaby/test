...
// Type: Softing.Dts.DtsDbEcuStateTransitionsImpl
...
internal class DtsDbEcuStateTransitionsImpl : 
  MappedObject,
  DtsDbEcuStateTransitions,
  ...
  DtsObject
{
  ...
  ~DtsDbEcuStateTransitionsImpl() => this.Dispose(false);
  ...
  public MCDDbEcuStateTransition GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbEcuStateTransitions_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbEcuStateTransition) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbEcuStateTransition);
  }
  ...
  public MCDDbEcuStateTransition GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbEcuStateTransitions_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbEcuStateTransition[] array = new MCDDbEcuStateTransition[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbEcuStateTransitionsImpl.DtsDbEcuStateTransitionsEnumerator(this);
  }
