...
// Type: Softing.Dts.DtsDbCodeInformationsImpl
...
internal class DtsDbCodeInformationsImpl : 
  MappedObject,
  DtsDbCodeInformations,
  ...
  DtsObject
{
  ...
  ~DtsDbCodeInformationsImpl() => this.Dispose(false);
  ...
  public MCDDbCodeInformation GetItemByIndex(uint uIndex)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbCodeInformations_getItemByIndex(this.Handle, uIndex, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbCodeInformation) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbCodeInformation);
  }
  ...
  public MCDDbCodeInformation GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbCodeInformations_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbCodeInformation[] array = new MCDDbCodeInformation[(int) this.Count];
    for (uint uIndex = 0; uIndex < this.Count; ++uIndex)
  ...
  {
    return (IEnumerator) new DtsDbCodeInformationsImpl.DtsDbCodeInformationsEnumerator(this);
  }
