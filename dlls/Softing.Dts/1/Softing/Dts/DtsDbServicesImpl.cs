...
// Type: Softing.Dts.DtsDbServicesImpl
...
internal class DtsDbServicesImpl : 
  MappedObject,
  DtsDbServices,
  MCDDbServices,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbServicesImpl() => this.Dispose(false);
  ...
  public MCDDbService GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbServices_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbService) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbService);
  }
  ...
  public MCDDbService GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbServices_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbService[] array = new MCDDbService[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbServicesImpl.DtsDbServicesEnumerator(this);
  }
