...
// Type: Softing.Dts.DtsDbEnvDataConnectorsImpl
...
internal class DtsDbEnvDataConnectorsImpl : 
  MappedObject,
  DtsDbEnvDataConnectors,
  ...
  DtsObject
{
  ...
  public DtsDbEnvDataConnectorsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbEnvDataConnectorsImpl() => this.Dispose(false);
  ...
  public MCDDbEnvDataConnector GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbEnvDataConnectors_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbEnvDataConnector) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbEnvDataConnector);
  }

  public MCDDbEnvDataConnector GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbEnvDataConnectors_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbEnvDataConnector> ToList()
  {
    List<MCDDbEnvDataConnector> list = new List<MCDDbEnvDataConnector>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbEnvDataConnector[] ToArray()
  {
    MCDDbEnvDataConnector[] array = new MCDDbEnvDataConnector[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbEnvDataConnectorsImpl.DtsDbEnvDataConnectorsEnumerator(this);
  }

  private class DtsDbEnvDataConnectorsEnumerator : IEnumerator
  {
    private DtsDbEnvDataConnectorsImpl m_classList;
    private int m_index;

    public DtsDbEnvDataConnectorsEnumerator(DtsDbEnvDataConnectorsImpl classList)
    {
