...
// Type: Softing.Dts.DtsDbFlashChecksumsImpl
...
internal class DtsDbFlashChecksumsImpl : 
  MappedObject,
  DtsDbFlashChecksums,
  MCDDbFlashChecksums,
  MCDNamedCollection,
  ...
  DtsObject
{
  ...
  ~DtsDbFlashChecksumsImpl() => this.Dispose(false);
  ...
  public MCDDbFlashChecksum GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFlashChecksums_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFlashChecksum) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFlashChecksum);
  }
  ...
  public MCDDbFlashChecksum GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFlashChecksums_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbFlashChecksum[] array = new MCDDbFlashChecksum[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFlashChecksumsImpl.DtsDbFlashChecksumsEnumerator(this);
  }
