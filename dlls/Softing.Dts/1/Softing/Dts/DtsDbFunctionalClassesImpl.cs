...
// Type: Softing.Dts.DtsDbFunctionalClassesImpl
...
internal class DtsDbFunctionalClassesImpl : 
  MappedObject,
  DtsDbFunctionalClasses,
  ...
  DtsObject
{
  ...
  public DtsDbFunctionalClassesImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbFunctionalClassesImpl() => this.Dispose(false);
  ...
  public MCDDbFunctionalClass GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFunctionalClasses_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFunctionalClass) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFunctionalClass);
  }

  public MCDDbFunctionalClass GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFunctionalClasses_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbFunctionalClass> ToList()
  {
    List<MCDDbFunctionalClass> list = new List<MCDDbFunctionalClass>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbFunctionalClass[] ToArray()
  {
    MCDDbFunctionalClass[] array = new MCDDbFunctionalClass[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFunctionalClassesImpl.DtsDbFunctionalClassesEnumerator(this);
  }

  private class DtsDbFunctionalClassesEnumerator : IEnumerator
  {
    private DtsDbFunctionalClassesImpl m_classList;
    private int m_index;

    public DtsDbFunctionalClassesEnumerator(DtsDbFunctionalClassesImpl classList)
    {
