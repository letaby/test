...
// Type: Softing.Dts.DtsDbFunctionDictionariesImpl
...
internal class DtsDbFunctionDictionariesImpl : 
  MappedObject,
  DtsDbFunctionDictionaries,
  ...
  DtsObject
{
  ...
  ~DtsDbFunctionDictionariesImpl() => this.Dispose(false);
  ...
  public MCDDbFunctionDictionary GetItemByIndex(uint index)
  {
    ...
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbFunctionDictionaries_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbFunctionDictionary) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbFunctionDictionary);
  }
  ...
  public MCDDbFunctionDictionary GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbFunctionDictionaries_getItemByName(this.Handle, name, out returnValue);
  ...
  {
    MCDDbFunctionDictionary[] array = new MCDDbFunctionDictionary[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbFunctionDictionariesImpl.DtsDbFunctionDictionariesEnumerator(this);
  }
