...
// Type: Softing.Dts.DtsDbProjectDescriptionsImpl
...
internal class DtsDbProjectDescriptionsImpl : 
  MappedObject,
  DtsDbProjectDescriptions,
  ...
  DtsObject
{
  ...
  public DtsDbProjectDescriptionsImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbProjectDescriptionsImpl() => this.Dispose(false);
  ...
  public MCDDbProjectDescription GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbProjectDescriptions_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbProjectDescription) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbProjectDescription);
  }

  public MCDDbProjectDescription GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbProjectDescriptions_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbProjectDescription> ToList()
  {
    List<MCDDbProjectDescription> list = new List<MCDDbProjectDescription>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbProjectDescription[] ToArray()
  {
    MCDDbProjectDescription[] array = new MCDDbProjectDescription[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbProjectDescriptionsImpl.DtsDbProjectDescriptionsEnumerator(this);
  }

  private class DtsDbProjectDescriptionsEnumerator : IEnumerator
  {
    private DtsDbProjectDescriptionsImpl m_classList;
    private int m_index;

    public DtsDbProjectDescriptionsEnumerator(DtsDbProjectDescriptionsImpl classList)
    {
