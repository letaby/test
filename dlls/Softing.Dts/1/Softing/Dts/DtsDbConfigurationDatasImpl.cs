...
// Type: Softing.Dts.DtsDbConfigurationDatasImpl
...
internal class DtsDbConfigurationDatasImpl : 
  MappedObject,
  DtsDbConfigurationDatas,
  ...
  DtsObject
{
  ...
  public DtsDbConfigurationDatasImpl(IntPtr handle)
  {
    this.Handle = handle;
  ...
  ~DtsDbConfigurationDatasImpl() => this.Dispose(false);
  ...
  public MCDDbConfigurationData GetItemByIndex(uint index)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByIndex = CSWrap.CSNIDTS_DtsDbConfigurationDatas_getItemByIndex(this.Handle, index, out returnValue);
    if (itemByIndex != IntPtr.Zero)
      throw DTS_ObjectMapper.createException(itemByIndex);
    return (MCDDbConfigurationData) (DTS_ObjectMapper.createObject(returnValue.m_handle, returnValue.m_type) as DtsDbConfigurationData);
  }

  public MCDDbConfigurationData GetItemByName(string name)
  {
    GC.KeepAlive((object) this);
    ObjectInfo_Struct returnValue = new ObjectInfo_Struct();
    IntPtr itemByName = CSWrap.CSNIDTS_DtsDbConfigurationDatas_getItemByName(this.Handle, name, out returnValue);
  ...
  public List<MCDDbConfigurationData> ToList()
  {
    List<MCDDbConfigurationData> list = new List<MCDDbConfigurationData>();
    for (uint index = 0; index < this.Count; ++index)
      list.Add(this.GetItemByIndex(index));
    return list;
  }

  public MCDDbConfigurationData[] ToArray()
  {
    MCDDbConfigurationData[] array = new MCDDbConfigurationData[(int) this.Count];
    for (uint index = 0; index < this.Count; ++index)
  ...
  {
    return (IEnumerator) new DtsDbConfigurationDatasImpl.DtsDbConfigurationDatasEnumerator(this);
  }

  private class DtsDbConfigurationDatasEnumerator : IEnumerator
  {
    private DtsDbConfigurationDatasImpl m_classList;
    private int m_index;

    public DtsDbConfigurationDatasEnumerator(DtsDbConfigurationDatasImpl classList)
    {
