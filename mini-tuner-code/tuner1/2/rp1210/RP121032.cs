...
public class RP121032 : IDisposable
{
	...
	public static List<string> ScanForDrivers()
	{
		...
		try
		{
			return new List<string>(new IniParser(iniPath).GetSetting("RP1210Support", "APIImplementations").Split(','));
		}
		catch
		{
			return null;
	...
	public static RP1210BDriverInfo LoadDeviceParameters(string strDeviceIniPath)
	{
		...
		try
		{
			...
			rP1210BDriverInfo.CANFormatsSupported = new List<short>(Array.ConvertAll(setting.Split(','), (string x) => Convert.ToInt16(x)));
		}
		catch
		{
		...
		try
		{
			...
			rP1210BDriverInfo.J1939FormatsSupported = new List<short>(Array.ConvertAll(setting2.Split(','), (string x) => Convert.ToInt16(x)));
		}
		catch
		{
		...
		try
		{
			...
			rP1210BDriverInfo.J1708FormatsSupported = new List<short>(Array.ConvertAll(setting3.Split(','), (string x) => Convert.ToInt16(x)));
		}
		catch
		{
		...
		try
		{
			...
			rP1210BDriverInfo.CANFormatsSupported = new List<short>(Array.ConvertAll(setting4.Split(','), (string x) => Convert.ToInt16(x)));
		}
		catch
		{
			rP1210BDriverInfo.ISO15765FormatsSupported = new List<short>(new short[1] { 2 });
		}
		foreach (string item in new List<string>(iniParser.GetSetting("VendorInformation", "Devices").Split(',')))
		{
		...
		foreach (string item2 in new List<string>(iniParser.GetSetting("VendorInformation", "Protocols").Split(',')))
		{
			...
			try
			{
				...
				protocolInfo.ProtocolSpeed = new List<string>(setting5.Split(','));
			}
			catch
			{
			}
			foreach (string devTemp in new List<string>(iniParser.GetSetting("ProtocolInformation" + item2, "Devices").Split(',')))
			{
	...
	public static byte[] EncodeJ1939Message(J1939Message MessageToEncode)
	{
		...
		array[b++] = (byte)(MessageToEncode.PGN & 0xFF);
		array[b++] = (byte)((MessageToEncode.PGN >> 8) & 0xFF);
		array[b++] = (byte)((MessageToEncode.PGN >> 16) & 0xFF);
